...
- une page publique `/weekly-preview` qui génère directement la semaine réelle suivante, affiche la slide 1 et les slides éditoriales retenues, avec partage/enregistrement de chaque image ;
- un délai réseau hebdomadaire de 30 secondes, séparé du délai quotidien de 10 secondes, pour laisser les cinq modèles répondre sans modifier le moteur quotidien ;
...
- Le renderer quotidien `src/ui/instagramOfficial24.ts` conserve son comportement et sa sortie ; son logo est désormais fourni par un asset partagé avec le renderer hebdomadaire.
...
## Correctifs issus de l’audit visuel

- Le défaut N1 de caractères minuscules manquants dans le Canvas hebdomadaire est corrigé par le même traitement texte que le quotidien : normalisation Unicode, pile Helvetica Neue, réglage du kerning et double rendu `strokeText`/`fillText`.
- Le point N2 réutilise la décision V24 quotidienne réelle pour chaque journée (`buildDayProfileV2` puis `chooseScene24V2`) ; un événement hebdomadaire ne force donc pas artificiellement une scène différente.
- Le carrousel réutilise le logo LOKA!, les box translucides, les pictogrammes redessinés, les fonds `masters24` et la signature « Ici, cette semaine. ».
- Les textes restent déterministes : formulation éditoriale resserrée, décimales françaises et conseils d’activité affichés seulement lorsqu’ils apportent une information exploitable.
- Ces correctifs sont couverts par les tests hebdomadaires et le flag `WEEKLY_ENABLED` reste désactivé.
