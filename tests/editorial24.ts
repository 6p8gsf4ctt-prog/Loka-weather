...
ok(p15.visual.secondaryLine.toLowerCase().includes('seconde partie de journée'),'scene15_secondary_timing');
...
ok(p21.visual.secondaryLine.startsWith('Temps sec'),'scene21_dry_context');
const p22=buildEditorialProductV2(CITIES.tarnos,classify(22).profile,classify(22).decision,15,20);
ok(p22.visual.secondaryLine.startsWith('Risque orageux'),'scene22_thunder_detail');
...
ok(p24.visual.secondaryLine.includes('mm')&&p24.visual.secondaryLine.includes('km/h'),'scene24_combined_detail');
