...
ok(JSON.stringify(EDITORIAL_ACTIVE_ENGAGEMENT_FIELDS) === JSON.stringify([]), "engagement_fields_retired");
ok(
  JSON.stringify(EDITORIAL_LEGACY_RETIRED_FIELDS) === JSON.stringify(["subtitle", "engagementFormat", "engagementQuestion", "engagementOptionA", "engagementOptionB"]),
  "legacy_retired_fields"
);
ok(editorialExportFieldStatus("subtitle") === "LEGACY_RETIRED", "subtitle_is_legacy");
ok(editorialExportFieldStatus("engagementFormat") === "LEGACY_RETIRED", "engagement_format_is_legacy");
ok(editorialExportFieldStatus("engagementQuestion") === "LEGACY_RETIRED", "engagement_question_is_legacy");
...
console.log(`EDITORIAL_EXPORT_STEP6 ${passed}/16 PASS`);
