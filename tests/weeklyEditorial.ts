...
ok(editorial.slide1.coldestMorning.label === "MATIN LE PLUS FRAIS" && editorial.slide1.coldestMorning.date === editorial.slide1.facts.coldestMorning.date && editorial.slide1.coldestMorning.temperatureC === editorial.slide1.facts.coldestMorning.temperatureC && editorial.slide1.coldestMorning.sourceHour >= 5 && editorial.slide1.coldestMorning.sourceHour <= 10 && editorial.slide1.coldestMorning.matches.every((match) => match.temperatureC === editorial.slide1.coldestMorning.temperatureC), "slide1_coldest_morning_is_traceable");
ok(editorial.slide1.hottestDay.label === "JOURNÉE LA PLUS CHAUDE" && editorial.slide1.hottestDay.date === editorial.slide1.facts.hottestDay.date && editorial.slide1.hottestDay.temperatureC === editorial.slide1.facts.hottestDay.temperatureC && editorial.slide1.hottestDay.sourceHour >= 0 && editorial.slide1.hottestDay.sourceHour <= 23 && editorial.slide1.hottestDay.matches.every((match) => match.temperatureC === editorial.slide1.hottestDay.temperatureC), "slide1_hottest_day_is_traceable");
ok(editorial.slide1.daylight.label === "LUMIÈRE DE LA SEMAINE" && editorial.slide1.daylight.deltaMinutes === editorial.slide1.facts.daylight.deltaMinutes && editorial.slide1.daylight.periodLabel === "LUN. 7 → DIM. 13" && editorial.slide1.daylight.start.sunriseLabel.includes(":") && editorial.slide1.daylight.start.weekdayLabel === "LUN." && editorial.slide1.daylight.end.weekdayLabel === "DIM.", "slide1_daylight_is_astronomical_and_formatted");
...
ok(editorial.slide1.synthesis.primaryLine.includes(" · ") && editorial.slide1.synthesis.primaryLine.length <= 80 && editorial.slide1.synthesis.primaryMaximumLines === 1 && editorial.slide1.synthesis.secondaryLine.endsWith(".") && editorial.slide1.synthesis.secondaryLine.length <= 120 && editorial.slide1.synthesis.secondaryMaximumLines === 2, "slide1_synthesis_uses_the_daily_two_line_editorial_contract");
ok(!/conseil|privilégier|surveiller|globalement|progressivement/i.test(`${editorial.slide1.synthesis.primaryLine} ${editorial.slide1.synthesis.secondaryLine}`), "slide1_synthesis_describes_the_week_without_advice_or_generic_filler");
...
ok(editorial.dailyCardDetails.every((card) => card.slots.map((slot) => slot.hour).join(",") === "8,12,16,20" && card.slots.every((slot) => slot.sourceHour === slot.hour)), "daily_card_uses_four_daily_engine_hourly_checkpoints");
ok(editorial.dailyCardDetails[0]?.slots.every((slot) => slot.condition === "soleil") === true && editorial.dailyCardDetails[1]?.slots.every((slot) => slot.condition === "pluie") === true, "daily_card_conditions_reuse_daily_condition_rule");
...
const unrelatedEditorial = buildWeeklyEditorial(profiles, unrelatedEventSelection, unrelatedEventActivities, city);
...
const calmEditorial = buildWeeklyEditorial(profiles, calmSelection, { insights: [] }, city);
