...
/** One normalized master shared by every weekly feed scene. */
export const WEEKLY_EDITORIAL_MASTER_GRID = {
...
/**
 * Weekly Story frame copied from the daily Story renderer. The header safe
 * area and first content baseline are intentionally identical to V24 daily.
 */
export const WEEKLY_STORY_MASTER_GRID = {
  ...
  main: { x: 44, y: 396, width: 992, height: 1338, bottom: 1734 },
  innerX: 30,
  signature: {
    x: 540,
    baseline: 1810,
...
/** Scene 1 uses sections inside the master glass panel, never stacked boxes. */
export const WEEKLY_SLIDE1_DAILY_FEED_GRID = {
  x: WEEKLY_EDITORIAL_MASTER_GRID.x,
  width: WEEKLY_EDITORIAL_MASTER_GRID.width,
  title: WEEKLY_EDITORIAL_MASTER_GRID.title,
  main: WEEKLY_EDITORIAL_MASTER_GRID.main,
  facts: { y: 356, height: 388, separatorY: 768 },
  summary: { y: 806, height: 124, separatorY: 953 },
  dailyStrip: {
    y: 970,
    height: 290,
    bottom: 1260
  },
  signature: WEEKLY_EDITORIAL_MASTER_GRID.signature
} as const;
...
export const WEEKLY_SLIDE2_DAILY_FEED_GRID = {
  ...
  width: WEEKLY_EDITORIAL_MASTER_GRID.width,
  title: WEEKLY_EDITORIAL_MASTER_GRID.title,
  numberBox: WEEKLY_EDITORIAL_MASTER_GRID.main,
  signature: WEEKLY_EDITORIAL_MASTER_GRID.signature
} as const;
...
export interface WeeklyCarouselRenderOptions extends WeeklyCarouselBuildOptions {
  ...
  /** @deprecated Story variants now belong to every rendered weekly slide. */
...
function renderSlideCard(slide: WeeklyCarouselSlide, position: number, total: number): string {
  ...
  const publication = slide.kind === "OVERVIEW"
    ? '<section class="format-block"><div class="format-head"><span>PUBLICATION</span><span>1080 × 1440</span></div><div class="canvas-wrap"><canvas class="carousel-canvas" data-weekly-format="publication" data-slide-index="' + (position - 1) + '" width="' + WEEKLY_CAROUSEL_WIDTH + '" height="' + WEEKLY_CAROUSEL_HEIGHT + '" aria-label="' + escapeHtml(label + " · publication") + '"></canvas></div><button class="secondary download-format" data-format="publication" data-slide-index="' + (position - 1) + '" type="button">Enregistrer / partager la publication</button></section>'
    : "";
  const storyClass = slide.kind === "OVERVIEW" ? "weekly-story-canvas" : "carousel-canvas";
  const story = '<section class="format-block"><div class="format-head"><span>STORY</span><span>1080 × 1920</span></div><div class="canvas-wrap"><canvas class="' + storyClass + '" data-weekly-format="story" data-slide-index="' + (position - 1) + '" width="' + WEEKLY_STORY_WIDTH + '" height="' + WEEKLY_STORY_HEIGHT + '" aria-label="' + escapeHtml(label + " · Story") + '"></canvas></div><button class="secondary download-format" data-format="story" data-slide-index="' + (position - 1) + '" type="button">Enregistrer / partager la Story</button></section>';
  return '<article class="slide-card" data-slide-index="' + (position - 1) + '"' + (slide.eventId ? ' data-event-id="' + escapeHtml(slide.eventId) + '"' : '') + '><div class="slide-head"><span>' +
    ...
    '</span></div><div class="format-grid">' + publication + story + '</div>' + (activities ? '<div class="activity-list">' + activities + "</div>" : "") + '</article>';
}
...
export function renderWeeklyCarousel(editorial: WeeklyEditorial, options: WeeklyCarouselRenderOptions = {}): string {
  ...
  const cards = includeSlides ? plan.slides.map((slide) => renderSlideCard(slide, slide.index + 1, contextualDemo ? 4 : plan.slides.length)).join("\n") : "";
  ...
  const previewNote = !includeSlides
    ...
    : contextualDemo
    ? "Laboratoire interne : les scènes éditoriales sont affichées dans leur format Story définitif."
    : "La scène 1 est disponible en Publication et en Story. Les scènes 2 et 3 sont disponibles uniquement en Story.";
...
'<style>:root{--ink:#12264a;--gold:#fdb515;--paper:#f3f1eb;--muted:#6f716f;--dark:#171715}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif}.wrap{max-width:1180px;margin:0 auto;padding:24px 18px 48px}.toolbar{background:#fff;border-radius:24px;padding:20px 22px;margin-bottom:22px;box-shadow:0 12px 40px rgba(18,38,74,.08)}.topline{display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{font-size:27px;font-weight:850;letter-spacing:.06em}.badge{font-size:11px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}.toolbar h1{font-size:28px;line-height:1.05;margin:20px 0 6px}.muted{font-size:13px;color:var(--muted)}.pilot{display:flex;justify-content:space-between;gap:14px;margin:16px 0 0;padding:12px 14px;border-radius:14px;font-size:12px;line-height:1.35}.pilot strong,.pilot span{display:block}.pilot span{margin-top:3px}.pilot small{align-self:center;white-space:nowrap}.pilot.pass{background:#edf7ef;color:#21613b}.pilot.review{background:#faf3e3;color:#7a5b16}.pilot.blocked{background:#f9e9e7;color:#8c302b}.carousel{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:22px}.slide-card{background:#fff;border-radius:24px;padding:14px;box-shadow:0 14px 46px rgba(18,38,74,.1)}.slide-head,.format-head{display:flex;justify-content:space-between;gap:8px;padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.format-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:18px}.format-block{min-width:0}.canvas-wrap{overflow:hidden;border-radius:18px;background:#d6d4cf}.canvas-wrap canvas{display:block;width:100%;height:auto}.activity-list{display:flex;flex-direction:column;gap:7px;margin:12px 2px 0}.activity{border-radius:11px;padding:8px 10px;font-size:12px;line-height:1.35}.activity.favorable{background:#edf7ef;color:#21613b}.activity.mixed{background:#faf3e3;color:#7a5b16}.activity.unfavorable{background:#f9e9e7;color:#8c302b}.secondary{width:100%;margin-top:12px;border:0;border-radius:12px;padding:12px 10px;background:#f0f0ed;color:var(--dark);font:650 12px/1 -apple-system,BlinkMacSystemFont,sans-serif;cursor:pointer}.note{text-align:center;color:var(--muted);font-size:11px;line-height:1.5;margin:22px auto 0;max-width:760px}</style></head><body><main class="wrap"><section class="toolbar"><div class="topline"><div class="brand">LOKA!</div><div class="badge">' + escapeHtml(previewBadge) + '</div></div><h1>' + escapeHtml(previewTitle) + '</h1><div class="muted">' + escapeHtml(range) + ' · ' + String(plan.slides.length) + ' slide' + (plan.slides.length > 1 ? 's' : '') + '</div>' + pilotPanel + '</section>' + carouselSection + '<p class="note">' + escapeHtml(previewNote) + '</p></main><script>';
  ...
  const script = [
    ...
    "function drawPlainTextLine(label,x,y,color,align){ctx.fillStyle=color;ctx.textAlign=align;ctx.textBaseline='alphabetic';ctx.fillText(label,x,y);}",
    ...
    `function drawSlide1Header(logo,date,width){drawLokaLogo(logo,${LOKA_DAILY_FEED_FRAME.header.logoX},${LOKA_DAILY_FEED_FRAME.header.logoCenterY},${LOKA_DAILY_FEED_FRAME.header.logoWidth},${LOKA_DAILY_FEED_FRAME.header.logoHeight});trackedText(String(plan.citySlug==='tarnos'?'TARNOS':plan.citySlug).toUpperCase(),${LOKA_DAILY_FEED_FRAME.header.cityX},${LOKA_DAILY_FEED_FRAME.header.cityBaseline},${LOKA_DAILY_FEED_FRAME.header.citySize},${LOKA_DAILY_FEED_FRAME.header.cityWeight},slide1Ink,${LOKA_DAILY_FEED_FRAME.header.cityTracking},'center');plainText(normalizeText(date),width-50,${LOKA_DAILY_FEED_FRAME.header.dateBaseline},${LOKA_DAILY_FEED_FRAME.header.dateSize},${LOKA_DAILY_FEED_FRAME.header.dateWeight},slide1Ink,'right');}`,
    ...
    `function drawWeeklyStoryHeader(logo,date){drawLokaLogo(logo,${WEEKLY_STORY_MASTER_GRID.header.logoX},${WEEKLY_STORY_MASTER_GRID.header.logoCenterY},${WEEKLY_STORY_MASTER_GRID.header.logoWidth},${WEEKLY_STORY_MASTER_GRID.header.logoHeight});trackedText(String(plan.citySlug==='tarnos'?'TARNOS':plan.citySlug).toUpperCase(),${WEEKLY_STORY_MASTER_GRID.header.cityX},${WEEKLY_STORY_MASTER_GRID.header.cityBaseline},${WEEKLY_STORY_MASTER_GRID.header.citySize},${WEEKLY_STORY_MASTER_GRID.header.cityWeight},slide1Ink,${WEEKLY_STORY_MASTER_GRID.header.cityTracking},'center');plainText(normalizeText(date),${WEEKLY_STORY_MASTER_GRID.header.dateX},${WEEKLY_STORY_MASTER_GRID.header.dateBaseline},${WEEKLY_STORY_MASTER_GRID.header.dateSize},${WEEKLY_STORY_MASTER_GRID.header.dateWeight},slide1Ink,'right');}`,
    ...
    `function drawOverviewTitle(content){const title=normalizeText(content.title),titleSize=fittedSize(title,870,56,44,760);text(title,${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 55},241,titleSize,760,slide1Ink,'left');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=5;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 58},263);ctx.lineTo(${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 138},263);ctx.stroke();ctx.restore();}`,
    `function drawStoryOverviewTitle(content){const title=normalizeText(content.title),titleSize=fittedSize(title,882,56,44,760);text(title,${WEEKLY_STORY_MASTER_GRID.title.x + 55},281,titleSize,760,slide1Ink,'left');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=5;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(${WEEKLY_STORY_MASTER_GRID.title.x + 58},303);ctx.lineTo(${WEEKLY_STORY_MASTER_GRID.title.x + 138},303);ctx.stroke();ctx.restore();}`,
    "function weeklyMainBox(x,y,w,h){box(x,y,w,h);ctx.save();rr(x+1,y+1,w-2,h-2,35);ctx.fillStyle='rgba(255,255,255,.055)';ctx.fill();ctx.restore();}",
    "function weeklySeparator(x1,y1,x2,y2,variant='INK'){ctx.save();ctx.strokeStyle=variant==='GLASS'?'rgba(255,255,255,.82)':'rgba(6,31,76,.20)';ctx.lineWidth=variant==='GLASS'?1.5:1.2;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(Math.round(x1)+.5,Math.round(y1)+.5);ctx.lineTo(Math.round(x2)+.5,Math.round(y2)+.5);ctx.stroke();ctx.restore();}",
    "function weeklyAccent(x,y,width=58){ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=4;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x+width,y);ctx.stroke();ctx.restore();}",
    "function metricParts(value){const label=normalizeText(value),match=label.match(/^([+−-]?\\s*\\d+(?:[,.]\\d+)?)(?:\\s*)(.*)$/);return match?{number:match[1].replace(/\\s+/g,''),unit:match[2]}:{number:label,unit:''};}",
    "function drawMetricValue(value,cx,baseline,maxWidth,numberSize,unitSize,weight=800){const parts=metricParts(value);ctx.save();font(numberSize,weight);const numberWidth=ctx.measureText(parts.number).width;font(unitSize,Math.max(650,weight-120));const unitWidth=parts.unit?ctx.measureText(parts.unit).width:0,gap=parts.unit?10:0,total=numberWidth+gap+unitWidth,scale=Math.min(1,maxWidth/Math.max(1,total)),nSize=Math.round(numberSize*scale),uSize=Math.round(unitSize*scale);font(nSize,weight);const nWidth=ctx.measureText(parts.number).width;font(uSize,Math.max(650,weight-120));const uWidth=parts.unit?ctx.measureText(parts.unit).width:0,renderTotal=nWidth+(parts.unit?10:0)+uWidth,start=cx-renderTotal/2;ctx.restore();text(parts.number,start,baseline,nSize,weight,slide1Ink,'left');if(parts.unit)text(parts.unit,start+nWidth+10,baseline,uSize,Math.max(650,weight-120),slide1Ink,'left');}",
    "function drawWeeklyEditorialBlock(primary,secondary,x,y,w){const main=normalizeText(primary),detail=normalizeText(secondary),maxWidth=w;if(main){const mainSize=fittedSize(main,maxWidth,29,22,740);text(main,x,y+mainSize,mainSize,740,slide1Ink,'left');weeklyAccent(x,y+mainSize+14,54);if(detail)completeWrap(detail,x,y+mainSize+57,maxWidth,29,22,17,620,slide1Ink,'left',2);}else{completeWrap(detail,x,y+58,maxWidth,34,28,20,700,slide1Ink,'left',2);}}",
    ...
    `function drawSlide1TemperatureFact(fact,icon,x,y,w,iconWidth,iconHeight){const labelSize=fittedSize(fact.label,w-42,19,15,760),dateSize=fittedSize(fact.dateLabel,w-42,30,18,720);trackedText(fact.label,x+w/2,y+50,labelSize,760,slide1Ink,.35,'center');weeklyAccent(x+w/2-23,y+68,46);drawImageCentered(icon,x+w/2,y+162,iconWidth,iconHeight);text(fact.dateLabel,x+w/2,y+274,dateSize,720,slide1Ink,'center');drawMetricValue(fact.temperatureLabel,x+w/2,y+354,w-34,68,44,800);}`,
    `function drawStorySlide1TemperatureFact(fact,icon,x,y,w,iconWidth,iconHeight){const labelSize=fittedSize(fact.label,w-42,20,15,760),dateSize=fittedSize(fact.dateLabel,w-42,31,19,720);trackedText(fact.label,x+w/2,y+62,labelSize,760,slide1Ink,.35,'center');weeklyAccent(x+w/2-23,y+82,46);drawImageCentered(icon,x+w/2,y+222,iconWidth,iconHeight);text(fact.dateLabel,x+w/2,y+382,dateSize,720,slide1Ink,'center');drawMetricValue(fact.temperatureLabel,x+w/2,y+474,w-34,72,46,800);}`,
    ...
    `function drawSlide1DaylightFact(content,daylightIcon,x,y,w){const labelSize=fittedSize(content.label,w-42,19,15,760),periodSize=fittedSize(content.periodLabel,w-42,30,18,720),deltaValue=daylightDeltaValue(content.deltaLabel);trackedText(content.label,x+w/2,y+50,labelSize,760,slide1Ink,.35,'center');weeklyAccent(x+w/2-23,y+68,46);drawImageCentered(daylightIcon,x+w/2,y+162,205,154);text(content.periodLabel,x+w/2,y+274,periodSize,720,slide1Ink,'center');drawMetricValue(deltaValue,x+w/2,y+354,w-34,68,43,800);}`,
    `function drawStorySlide1DaylightFact(content,daylightIcon,x,y,w){const labelSize=fittedSize(content.label,w-42,20,15,760),periodSize=fittedSize(content.periodLabel,w-42,31,19,720),deltaValue=daylightDeltaValue(content.deltaLabel);trackedText(content.label,x+w/2,y+62,labelSize,760,slide1Ink,.35,'center');weeklyAccent(x+w/2-23,y+82,46);drawImageCentered(daylightIcon,x+w/2,y+222,215,164);text(content.periodLabel,x+w/2,y+382,periodSize,720,slide1Ink,'center');drawMetricValue(deltaValue,x+w/2,y+474,w-34,72,46,800);}`,
    ...
    `function drawSlide1Facts(content,dailyIcons,daylightIcon,thermometerIcon){const x=${WEEKLY_SLIDE1_DAILY_FEED_GRID.x},y=${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.y},w=${WEEKLY_SLIDE1_DAILY_FEED_GRID.width},cards=slide1FactLayout(content,x,w),hotIcon=dailyIcons[content.hottestDay.dayIndex];weeklySeparator(cards[1].x,y+4,cards[1].x,y+${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.height},'INK');weeklySeparator(cards[2].x,y+4,cards[2].x,y+${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.height},'INK');drawSlide1TemperatureFact(content.coldestMorning,thermometerIcon,cards[0].x,y,cards[0].width,155,140);drawSlide1TemperatureFact(content.hottestDay,hotIcon,cards[1].x,y,cards[1].width,195,165);drawSlide1DaylightFact(content.daylight,daylightIcon,cards[2].x,y,cards[2].width);}`,
    `function drawStorySlide1Facts(content,dailyIcons,daylightIcon,thermometerIcon){const x=${WEEKLY_STORY_MASTER_GRID.main.x},y=424,w=${WEEKLY_STORY_MASTER_GRID.main.width},cards=slide1FactLayout(content,x,w),hotIcon=dailyIcons[content.hottestDay.dayIndex];weeklySeparator(cards[1].x,y+6,cards[1].x,y+500,'INK');weeklySeparator(cards[2].x,y+6,cards[2].x,y+500,'INK');drawStorySlide1TemperatureFact(content.coldestMorning,thermometerIcon,cards[0].x,y,cards[0].width,170,154);drawStorySlide1TemperatureFact(content.hottestDay,hotIcon,cards[1].x,y,cards[1].width,210,176);drawStorySlide1DaylightFact(content.daylight,daylightIcon,cards[2].x,y,cards[2].width);}`,
    `function drawSlide1Summary(content){const x=${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 62},y=${WEEKLY_SLIDE1_DAILY_FEED_GRID.summary.y},w=${WEEKLY_SLIDE1_DAILY_FEED_GRID.width - 124},visual=content.synthesis;drawWeeklyEditorialBlock(visual.primaryLine,visual.secondaryLine,x,y,w);}`,
    `function drawStorySlide1Summary(content){const x=${WEEKLY_STORY_MASTER_GRID.main.x + 62},y=985,w=${WEEKLY_STORY_MASTER_GRID.main.width - 124},visual=content.synthesis;drawWeeklyEditorialBlock(visual.primaryLine,visual.secondaryLine,x,y,w);}`,
    "function factMatches(fact){return Array.isArray(fact.matches)&&fact.matches.length?fact.matches:[fact];}",
    "function drawSlide1DayMarker(kind,x,y,w,h){const accent=kind==='HOT'?gold:slide1ColdBlue,left=Math.round(x+8),top=Math.round(y+8),markerWidth=Math.round(w-16),markerHeight=Math.round(h-16);ctx.save();rr(left,top,markerWidth,markerHeight,12);ctx.fillStyle=kind==='HOT'?'rgba(253,181,21,.08)':'rgba(95,167,231,.08)';ctx.fill();ctx.strokeStyle=accent;ctx.lineWidth=1.5;ctx.stroke();ctx.restore();}",
    `function drawSlide1DayStrip(days,dailyIcons,content){const x=${WEEKLY_SLIDE1_DAILY_FEED_GRID.x},y=${WEEKLY_SLIDE1_DAILY_FEED_GRID.dailyStrip.y},w=${WEEKLY_SLIDE1_DAILY_FEED_GRID.width},h=${WEEKLY_SLIDE1_DAILY_FEED_GRID.dailyStrip.height},columnWidth=w/days.length,coldDayIndexes=new Set(factMatches(content.coldestMorning).map(match=>match.dayIndex)),hotDayIndexes=new Set(factMatches(content.hottestDay).map(match=>match.dayIndex));for(let index=1;index<days.length;index++)weeklySeparator(x+columnWidth*index,y+18,x+columnWidth*index,y+h-18,'INK');days.forEach(function(day,index){if(coldDayIndexes.has(day.dayIndex))drawSlide1DayMarker('COLD',x+columnWidth*index,y,columnWidth,h);if(hotDayIndexes.has(day.dayIndex))drawSlide1DayMarker('HOT',x+columnWidth*index,y,columnWidth,h);});days.forEach(function(day,index){const centerX=x+columnWidth*(index+.5),icon=dailyIcons[index];text(day.weekdayLabel,centerX,y+43,17,760,slide1Ink,'center');text(day.dayLabel,centerX,y+70,19,700,slide1Ink,'center');drawImageCentered(icon,centerX,y+150,106,82);const minLabel=String(Math.round(day.minTemperatureC))+'°',maxLabel=String(Math.round(day.maxTemperatureC))+'°';text(minLabel,centerX-7,y+242,24,700,slide1Ink,'right');text('/',centerX,y+242,17,520,slide1Ink,'center');text(maxLabel,centerX+7,y+242,24,700,slide1Ink,'left');});}`,
    `function drawStorySlide1DayStrip(days,dailyIcons,content){const x=${WEEKLY_STORY_MASTER_GRID.main.x},y=1185,w=${WEEKLY_STORY_MASTER_GRID.main.width},h=520,columnWidth=w/days.length,coldDayIndexes=new Set(factMatches(content.coldestMorning).map(match=>match.dayIndex)),hotDayIndexes=new Set(factMatches(content.hottestDay).map(match=>match.dayIndex));for(let index=1;index<days.length;index++)weeklySeparator(x+columnWidth*index,y+24,x+columnWidth*index,y+h-24,'INK');days.forEach(function(day,index){if(coldDayIndexes.has(day.dayIndex))drawSlide1DayMarker('COLD',x+columnWidth*index,y,columnWidth,h);if(hotDayIndexes.has(day.dayIndex))drawSlide1DayMarker('HOT',x+columnWidth*index,y,columnWidth,h);});days.forEach(function(day,index){const centerX=x+columnWidth*(index+.5),icon=dailyIcons[index];text(day.weekdayLabel,centerX,y+82,18,760,slide1Ink,'center');text(day.dayLabel,centerX,y+114,20,700,slide1Ink,'center');drawImageCentered(icon,centerX,y+264,112,88);const minLabel=String(Math.round(day.minTemperatureC))+'°',maxLabel=String(Math.round(day.maxTemperatureC))+'°';text(minLabel,centerX-7,y+432,25,700,slide1Ink,'right');text('/',centerX,y+432,18,520,slide1Ink,'center');text(maxLabel,centerX+7,y+432,25,700,slide1Ink,'left');});}`,
    ...
    `function drawWeeklyStorySignature(){text(model.signature||model.brand.slogan,${WEEKLY_STORY_MASTER_GRID.signature.x},${WEEKLY_STORY_MASTER_GRID.signature.baseline},${WEEKLY_STORY_MASTER_GRID.signature.size},${WEEKLY_STORY_MASTER_GRID.signature.weight},rgba(ink,${WEEKLY_STORY_MASTER_GRID.signature.colorAlpha}),'center');ctx.save();ctx.strokeStyle=gold;ctx.lineWidth=${WEEKLY_STORY_MASTER_GRID.signature.underlineWidth};ctx.lineCap='round';ctx.beginPath();ctx.moveTo(${WEEKLY_STORY_MASTER_GRID.signature.underlineStartX},${WEEKLY_STORY_MASTER_GRID.signature.underlineY});ctx.lineTo(${WEEKLY_STORY_MASTER_GRID.signature.underlineEndX},${WEEKLY_STORY_MASTER_GRID.signature.underlineY});ctx.stroke();ctx.restore();}`,
    ...
    `function drawWeeklyStoryFrame(canvas,background,logo,titleContent){ctx=canvas.getContext('2d');ctx.clearRect(0,0,${WEEKLY_STORY_WIDTH},${WEEKLY_STORY_HEIGHT});cover(background,${WEEKLY_STORY_WIDTH},${WEEKLY_STORY_HEIGHT});overlay(${WEEKLY_STORY_WIDTH},${WEEKLY_STORY_HEIGHT});drawWeeklyStoryHeader(logo,plan.headerDateFeed);box(${WEEKLY_STORY_MASTER_GRID.title.x},${WEEKLY_STORY_MASTER_GRID.title.y},${WEEKLY_STORY_MASTER_GRID.title.width},${WEEKLY_STORY_MASTER_GRID.title.height});drawStoryOverviewTitle(titleContent);}`,
    `function drawSlide1Overview(canvas,slide,background,logo,dailyIcons,daylightIcon,thermometerIcon){drawWeeklySharedFrame(canvas,background,logo,plan.overviewTitle);weeklyMainBox(${WEEKLY_EDITORIAL_MASTER_GRID.x},${WEEKLY_EDITORIAL_MASTER_GRID.main.y},${WEEKLY_EDITORIAL_MASTER_GRID.width},${WEEKLY_EDITORIAL_MASTER_GRID.main.height});drawSlide1Facts(plan.slide1,dailyIcons,daylightIcon,thermometerIcon);weeklySeparator(${WEEKLY_EDITORIAL_MASTER_GRID.x + WEEKLY_EDITORIAL_MASTER_GRID.innerX},${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.separatorY},${WEEKLY_EDITORIAL_MASTER_GRID.x + WEEKLY_EDITORIAL_MASTER_GRID.width - WEEKLY_EDITORIAL_MASTER_GRID.innerX},${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.separatorY},'GLASS');drawSlide1Summary(plan.slide1);weeklySeparator(${WEEKLY_EDITORIAL_MASTER_GRID.x + WEEKLY_EDITORIAL_MASTER_GRID.innerX},${WEEKLY_SLIDE1_DAILY_FEED_GRID.summary.separatorY},${WEEKLY_EDITORIAL_MASTER_GRID.x + WEEKLY_EDITORIAL_MASTER_GRID.width - WEEKLY_EDITORIAL_MASTER_GRID.innerX},${WEEKLY_SLIDE1_DAILY_FEED_GRID.summary.separatorY},'GLASS');drawSlide1DayStrip(plan.slide1.dailyStrip,dailyIcons,plan.slide1);drawSlide1FeedSignature();}`,
    `function drawSlide1OverviewStory(canvas,slide,background,logo,dailyIcons,daylightIcon,thermometerIcon){drawWeeklyStoryFrame(canvas,background,logo,plan.overviewTitle);weeklyMainBox(${WEEKLY_STORY_MASTER_GRID.main.x},${WEEKLY_STORY_MASTER_GRID.main.y},${WEEKLY_STORY_MASTER_GRID.main.width},${WEEKLY_STORY_MASTER_GRID.main.height});drawStorySlide1Facts(plan.slide1,dailyIcons,daylightIcon,thermometerIcon);weeklySeparator(${WEEKLY_STORY_MASTER_GRID.main.x + WEEKLY_STORY_MASTER_GRID.innerX},946,${WEEKLY_STORY_MASTER_GRID.main.x + WEEKLY_STORY_MASTER_GRID.main.width - WEEKLY_STORY_MASTER_GRID.innerX},946,'GLASS');drawStorySlide1Summary(plan.slide1);weeklySeparator(${WEEKLY_STORY_MASTER_GRID.main.x + WEEKLY_STORY_MASTER_GRID.innerX},1165,${WEEKLY_STORY_MASTER_GRID.main.x + WEEKLY_STORY_MASTER_GRID.main.width - WEEKLY_STORY_MASTER_GRID.innerX},1165,'GLASS');drawStorySlide1DayStrip(plan.slide1.dailyStrip,dailyIcons,plan.slide1);drawWeeklyStorySignature();}`,
    ...
    `function drawComparisonItem(item,cx,valueY,labelY,maxWidth){drawMetricValue(item.value,cx,valueY,maxWidth,58,39,800);completeWrap(item.label,cx,labelY,maxWidth,25,22,18,700,slide1Ink,'center',2);}`,
    `function drawComplementaryContent(content,icon,x,y,w){const visual=content.presentation,innerX=x+${WEEKLY_EDITORIAL_MASTER_GRID.innerX},innerRight=x+w-${WEEKLY_EDITORIAL_MASTER_GRID.innerX};drawImageCentered(icon,x+w/2,y+145,170,138);drawMetricValue(visual.headline,x+w/2,y+372,w-130,124,82,820);completeWrap(visual.subtitle,x+w/2,y+450,w-110,34,31,24,780,slide1Ink,'center',2);weeklyAccent(x+w/2-40,y+492,80);weeklySeparator(innerX,y+530,innerRight,y+530,'GLASS');if(visual.comparison){const leftX=x+w*.28,rightX=x+w*.72,itemWidth=w*.36;drawComparisonItem(visual.comparison.left,leftX,y+646,y+700,itemWidth);drawComparisonItem(visual.comparison.right,rightX,y+646,y+700,itemWidth);weeklySeparator(x+w/2,y+568,x+w/2,y+724,'INK');weeklySeparator(innerX,y+762,innerRight,y+762,'GLASS');drawWeeklyEditorialBlock('',visual.editorialLine,x+72,y+804,w-144);}else{weeklySeparator(innerX,y+630,innerRight,y+630,'GLASS');drawWeeklyEditorialBlock('',visual.editorialLine,x+72,y+690,w-144);}}`,
    `function drawStoryComplementaryContent(content,icon,x,y,w){const visual=content.presentation,innerX=x+${WEEKLY_STORY_MASTER_GRID.innerX},innerRight=x+w-${WEEKLY_STORY_MASTER_GRID.innerX};drawImageCentered(icon,x+w/2,y+190,184,148);drawMetricValue(visual.headline,x+w/2,y+478,w-130,132,86,820);completeWrap(visual.subtitle,x+w/2,y+570,w-110,38,34,25,780,slide1Ink,'center',2);weeklyAccent(x+w/2-40,y+626,80);weeklySeparator(innerX,y+682,innerRight,y+682,'GLASS');if(visual.comparison){const leftX=x+w*.28,rightX=x+w*.72,itemWidth=w*.36;drawComparisonItem(visual.comparison.left,leftX,y+842,y+904,itemWidth);drawComparisonItem(visual.comparison.right,rightX,y+842,y+904,itemWidth);weeklySeparator(x+w/2,y+744,x+w/2,y+944,'INK');weeklySeparator(innerX,y+1000,innerRight,y+1000,'GLASS');drawWeeklyEditorialBlock('',visual.editorialLine,x+72,y+1080,w-144);}else{weeklySeparator(innerX,y+838,innerRight,y+838,'GLASS');drawWeeklyEditorialBlock('',visual.editorialLine,x+72,y+930,w-144);}}`,
    `function drawComplementarySlide(canvas,slide,background,logo,icon){const content=slide.complementary,x=${WEEKLY_COMPLEMENTARY_DAILY_FEED_GRID.x},y=${WEEKLY_COMPLEMENTARY_DAILY_FEED_GRID.numberBox.y},w=${WEEKLY_COMPLEMENTARY_DAILY_FEED_GRID.width},h=${WEEKLY_COMPLEMENTARY_DAILY_FEED_GRID.numberBox.height};if(!content||content.frame!=='WEEKLY_SHARED_V1'||!content.presentation)throw new Error('weekly_shared_frame_required');drawWeeklySharedFrame(canvas,background,logo,content);weeklyMainBox(x,y,w,h);drawComplementaryContent(content,icon,x,y,w);drawSlide1FeedSignature();}`,
    `function drawComplementaryStory(canvas,slide,background,logo,icon){const content=slide.complementary,x=${WEEKLY_STORY_MASTER_GRID.main.x},y=${WEEKLY_STORY_MASTER_GRID.main.y},w=${WEEKLY_STORY_MASTER_GRID.main.width},h=${WEEKLY_STORY_MASTER_GRID.main.height};if(!content||content.frame!=='WEEKLY_SHARED_V1'||!content.presentation)throw new Error('weekly_shared_story_frame_required');drawWeeklyStoryFrame(canvas,background,logo,content);weeklyMainBox(x,y,w,h);drawStoryComplementaryContent(content,icon,x,y,w);drawWeeklyStorySignature();}`,
    `function drawSlide2NumberStory(canvas,slide,background,logo,icon){const content=slide.weeklyNumber,x=${WEEKLY_STORY_MASTER_GRID.main.x},y=${WEEKLY_STORY_MASTER_GRID.main.y},w=${WEEKLY_STORY_MASTER_GRID.main.width},h=${WEEKLY_STORY_MASTER_GRID.main.height};drawWeeklyStoryFrame(canvas,background,logo,content);weeklyMainBox(x,y,w,h);drawImageCentered(icon,x+w/2,y+244,238,184);drawMetricValue(content.valueLabel,x+w/2,y+570,w-120,148,92,820);completeWrap(content.unitLabel,x+w/2,y+652,w-120,36,30,22,760,slide1Ink,'center',2);weeklyAccent(x+w/2-40,y+704,80);weeklySeparator(x+${WEEKLY_STORY_MASTER_GRID.innerX},y+770,x+w-${WEEKLY_STORY_MASTER_GRID.innerX},y+770,'GLASS');drawWeeklyEditorialBlock('',content.explanation,x+72,y+870,w-144);drawWeeklyStorySignature();}`,
    ...
    "function drawWeeklyStorySlide(canvas,slide){if(slide.kind==='WEEKLY_NUMBER')return Promise.all([load(slide.backgroundUrl,'story_number_background'),load(model.brand.logoUrl,'story_number_logo'),load(model.weeklyNumberPictogram,'story_number_pictogram')]).then(function(images){drawSlide2NumberStory(canvas,slide,images[0],images[1],images[2]);});if(slide.kind==='COMPLEMENTARY_NUMBER'||slide.kind==='COMPLEMENTARY_PRACTICAL'||slide.kind==='COMPLEMENTARY_DETAIL')return Promise.all([load(slide.backgroundUrl,'story_contextual_background_'+slide.index),load(model.brand.logoUrl,'story_contextual_logo_'+slide.index),load(slide.complementaryPictogramUrl,'story_contextual_pictogram_'+slide.index)]).then(function(images){drawComplementaryStory(canvas,slide,images[0],images[1],images[2]);});const base=[load(slide.backgroundUrl,'story_slide1_background'),load(model.brand.logoUrl,'story_slide1_logo')],dailyPictograms=plan.slide1.dailyStrip.map(function(day){return load(day.pictogram.url,'story_slide1_pictogram_'+day.dayIndex);}),utilityPictograms=[load(model.slide1UtilityPictograms.daylight,'story_slide1_daylight'),load(model.slide1UtilityPictograms.thermometer,'story_slide1_thermometer')];return Promise.all([...base,...dailyPictograms,...utilityPictograms]).then(function(images){const dailyOffset=2,utilityOffset=dailyOffset+dailyPictograms.length;drawSlide1OverviewStory(canvas,slide,images[0],images[1],images.slice(dailyOffset,utilityOffset),images[utilityOffset],images[utilityOffset+1]);});}",
    ...
    "Promise.all(publicationCanvases.map(function(canvas){const index=Number(canvas.dataset.slideIndex||0),slide=plan.slides[index];return drawPublicationSlide(canvas,slide);})).catch(function(error){console.error(error);});",
    "Promise.all(storyCanvases.map(function(canvas){const index=Number(canvas.dataset.slideIndex||0),slide=plan.slides[index];return drawWeeklyStorySlide(canvas,slide);})).catch(function(error){console.error(error);});",
    "document.querySelectorAll('.download-format').forEach(function(button){button.addEventListener('click',function(){const index=Number(button.dataset.slideIndex||0),format=String(button.dataset.format||'story'),slide=plan.slides[index],canvas=format==='publication'?publicationCanvases.find(item=>Number(item.dataset.slideIndex||0)===index):storyCanvases.find(item=>Number(item.dataset.slideIndex||0)===index);if(canvas)shareCanvas(canvas,'loka-semaine-'+String((slide?slide.index:index)+1).padStart(2,'0')+'-'+format+'.png');});});"
  ].join("\n");
