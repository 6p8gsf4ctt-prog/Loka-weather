...
import { LOKA_DAILY_STORY_FRAME, LOKA_WEEKLY_STORY_ADAPTIVE_LAYOUT } from "../../ui/storyFrame";
...
/** The weekly Story consumes the exact same permanent frame as the daily Story. */
export const WEEKLY_SLIDE1_DAILY_STORY_GRID = {
  x: LOKA_DAILY_STORY_FRAME.title.x,
  width: LOKA_DAILY_STORY_FRAME.title.width,
  title: LOKA_DAILY_STORY_FRAME.title,
  content: LOKA_DAILY_STORY_FRAME.content,
  signature: LOKA_DAILY_STORY_FRAME.signature,
  adaptive: LOKA_WEEKLY_STORY_ADAPTIVE_LAYOUT
} as const;
...
'<style>:root{--ink:#12264a;--gold:#fdb515;--paper:#f3f1eb;--muted:#6f716f;--dark:#171715}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif}.wrap{max-width:1180px;margin:0 auto;padding:24px 18px 48px}.toolbar{background:#fff;border-radius:24px;padding:20px 22px;margin-bottom:22px;box-shadow:0 12px 40px rgba(18,38,74,.08)}.topline{display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{font-size:27px;font-weight:850;letter-spacing:.06em}.badge{font-size:11px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}.toolbar h1{font-size:28px;line-height:1.05;margin:20px 0 6px}.muted{font-size:13px;color:var(--muted)}.pilot{display:flex;justify-content:space-between;gap:14px;margin:16px 0 0;padding:12px 14px;border-radius:14px;font-size:12px;line-height:1.35}.pilot strong,.pilot span{display:block}.pilot span{margin-top:3px}.pilot small{align-self:center;white-space:nowrap}.pilot.pass{background:#edf7ef;color:#21613b}.pilot.review{background:#faf3e3;color:#7a5b16}.pilot.blocked{background:#f9e9e7;color:#8c302b}.carousel{display:grid;gap:22px}.slide-card{background:#fff;border-radius:24px;padding:14px;box-shadow:0 14px 46px rgba(18,38,74,.1)}.slide-head{display:flex;justify-content:space-between;gap:8px;padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.format-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:18px}.format-panel{min-width:0}.format-label{padding:0 4px 9px;font-size:10px;font-weight:800;letter-spacing:.1em;color:var(--muted)}.canvas-wrap{overflow:hidden;border-radius:18px;background:#d6d4cf}.canvas-wrap canvas{display:block;width:100%;height:auto}.story-wrap{max-width:420px;margin:0 auto}.activity-list{display:flex;flex-direction:column;gap:7px;margin:12px 2px 0}.activity{border-radius:11px;padding:8px 10px;font-size:12px;line-height:1.35}.activity.favorable{background:#edf7ef;color:#21613b}.activity.mixed{background:#faf3e3;color:#7a5b16}.activity.unfavorable{background:#f9e9e7;color:#8c302b}.secondary{width:100%;margin-top:12px;border:0;border-radius:12px;padding:12px 10px;background:#f0f0ed;color:var(--dark);font:650 12px/1 -apple-system,BlinkMacSystemFont,sans-serif;cursor:pointer}.note{text-align:center;color:var(--muted);font-size:11px;line-height:1.5;margin:22px auto 0;max-width:760px}</style></head><body><main class="wrap"><section class="toolbar"><div class="topline"><div class="brand">LOKA!</div><div class="badge">' + escapeHtml(previewBadge) + '</div></div><h1>' + escapeHtml(previewTitle) + '</h1><div class="muted">' + escapeHtml(range) + ' · ' + String(plan.slides.length) + ' slide' + (plan.slides.length > 1 ? 's' : '') + '</div>' + pilotPanel + '</section>' + carouselSection + storySection + '<p class="note">' + escapeHtml(previewNote) + '</p></main><script>';
  ...
  const script = [
    ...
    `function drawSlide1Header(logo,date,width){drawLokaLogo(logo,${LOKA_DAILY_FEED_FRAME.header.logoX},${LOKA_DAILY_FEED_FRAME.header.logoCenterY},${LOKA_DAILY_FEED_FRAME.header.logoWidth},${LOKA_DAILY_FEED_FRAME.header.logoHeight});trackedText(String(plan.citySlug==='tarnos'?'TARNOS':plan.citySlug).toUpperCase(),${LOKA_DAILY_FEED_FRAME.header.cityX},${LOKA_DAILY_FEED_FRAME.header.cityBaseline},${LOKA_DAILY_FEED_FRAME.header.citySize},${LOKA_DAILY_FEED_FRAME.header.cityWeight},ink,${LOKA_DAILY_FEED_FRAME.header.cityTracking},'center');text(normalizeText(date),width-50,${LOKA_DAILY_FEED_FRAME.header.dateBaseline},${LOKA_DAILY_FEED_FRAME.header.dateSize},${LOKA_DAILY_FEED_FRAME.header.dateWeight},ink,'right');}`,
    ...
    `function drawOverviewTitle(content){const title=normalizeText(content.title),titleSize=fittedSize(title,870,48,32,800);text(title,${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 55},241,titleSize,800,ink,'left');ctx.save();ctx.strokeStyle=gold;ctx.lineWidth=3;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 58},263);ctx.lineTo(${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 110},263);ctx.stroke();ctx.restore();}`,
    ...
    `function drawSlide1TemperatureFact(fact,icon,x,y,w,iconWidth,iconHeight){const labelSize=fittedSize(fact.label,w-44,19,15,700),dateSize=fittedSize(fact.dateLabel,w-44,30,18,560);trackedText(fact.label,x+w/2,y+72,labelSize,700,slide1Ink,.35,'center');drawFactAccent(x+w/2,y+92);drawImageCentered(icon,x+w/2,y+205,iconWidth,iconHeight);plainText(fact.dateLabel,x+w/2,y+329,dateSize,560,slide1Ink,'center');plainText(fact.temperatureLabel,x+w/2,y+430,fittedSize(fact.temperatureLabel,w-28,70,56,700),700,slide1Ink,'center');}`,
    ...
    `function drawSlide1DaylightFact(content,daylightIcon,x,y,w){const labelSize=fittedSize(content.label,w-44,19,15,700),periodSize=fittedSize(content.periodLabel,w-44,30,18,560),deltaValue=daylightDeltaValue(content.deltaLabel);trackedText(content.label,x+w/2,y+72,labelSize,700,slide1Ink,.35,'center');drawFactAccent(x+w/2,y+92);drawImageCentered(daylightIcon,x+w/2,y+205,205,154);plainText(content.periodLabel,x+w/2,y+329,periodSize,560,slide1Ink,'center');plainText(deltaValue,x+w/2,y+430,fittedSize(deltaValue,w-28,70,56,700),700,slide1Ink,'center');}`,
    ...
    `function drawSlide1Facts(content,dailyIcons,daylightIcon,thermometerIcon){const x=${WEEKLY_SLIDE1_DAILY_FEED_GRID.x},y=${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.y},w=${WEEKLY_SLIDE1_DAILY_FEED_GRID.width},h=${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.height},cards=slide1FactLayout(content,x,w),hotIcon=dailyIcons[content.hottestDay.dayIndex];slide1MetricBox(x,y,w,h);ctx.save();ctx.strokeStyle='rgba(6,31,76,.18)';ctx.lineWidth=1.2;for(let index=1;index<3;index++){const separatorX=x+w*index/3;ctx.beginPath();ctx.moveTo(separatorX,y+22);ctx.lineTo(separatorX,y+h-22);ctx.stroke();}ctx.restore();drawSlide1TemperatureFact(content.coldestMorning,thermometerIcon,cards[0].x,y,cards[0].width,155,140);drawSlide1TemperatureFact(content.hottestDay,hotIcon,cards[1].x,y,cards[1].width,195,165);drawSlide1DaylightFact(content.daylight,daylightIcon,cards[2].x,y,cards[2].width);}`,
    ...
    `function drawSlide1DayStrip(days,dailyIcons,content){const x=${WEEKLY_SLIDE1_DAILY_FEED_GRID.x},y=${WEEKLY_SLIDE1_DAILY_FEED_GRID.dailyStrip.y},w=${WEEKLY_SLIDE1_DAILY_FEED_GRID.width},h=${WEEKLY_SLIDE1_DAILY_FEED_GRID.dailyStrip.height},columnWidth=w/days.length,coldDayIndexes=new Set(factMatches(content.coldestMorning).map(match=>match.dayIndex)),hotDayIndexes=new Set(factMatches(content.hottestDay).map(match=>match.dayIndex));ctx.save();ctx.strokeStyle='rgba(6,31,76,.14)';ctx.lineWidth=1;for(let index=1;index<days.length;index++){const separatorX=Math.round(x+columnWidth*index)+.5;ctx.beginPath();ctx.moveTo(separatorX,y+18);ctx.lineTo(separatorX,y+h-18);ctx.stroke();}ctx.restore();days.forEach(function(day,index){if(coldDayIndexes.has(day.dayIndex))drawSlide1DayMarker('COLD',x+columnWidth*index,y,columnWidth,h);if(hotDayIndexes.has(day.dayIndex))drawSlide1DayMarker('HOT',x+columnWidth*index,y,columnWidth,h);});days.forEach(function(day,index){const centerX=x+columnWidth*(index+.5),icon=dailyIcons[index];plainText(day.weekdayLabel,centerX,y+40,17,700,slide1Ink,'center');plainText(day.dayLabel,centerX,y+66,19,560,slide1Ink,'center');drawImageCentered(icon,centerX,y+133,106,82);const minLabel=String(Math.round(day.minTemperatureC))+'°',maxLabel=String(Math.round(day.maxTemperatureC))+'°';plainText(minLabel,centerX-7,y+210,24,600,slide1Ink,'right');plainText('/',centerX,y+210,17,450,slide1Ink,'center');plainText(maxLabel,centerX+7,y+210,24,600,slide1Ink,'left');});}`,
    ...
    `function drawSlide1StoryHeader(logo){drawLokaLogo(logo,${LOKA_DAILY_STORY_FRAME.header.logoX},${LOKA_DAILY_STORY_FRAME.header.logoCenterY},${LOKA_DAILY_STORY_FRAME.header.logoWidth},${LOKA_DAILY_STORY_FRAME.header.logoHeight});trackedText(String(plan.citySlug==='tarnos'?'TARNOS':plan.citySlug).toUpperCase(),${LOKA_DAILY_STORY_FRAME.header.cityX},${LOKA_DAILY_STORY_FRAME.header.cityBaseline},${LOKA_DAILY_STORY_FRAME.header.citySize},${LOKA_DAILY_STORY_FRAME.header.cityWeight},slide1Ink,${LOKA_DAILY_STORY_FRAME.header.cityTracking},'center');plainText(normalizeText(plan.headerDateFeed),${LOKA_DAILY_STORY_FRAME.header.dateX},${LOKA_DAILY_STORY_FRAME.header.dateBaseline},${LOKA_DAILY_STORY_FRAME.header.dateSize},${LOKA_DAILY_STORY_FRAME.header.dateWeight},slide1Ink,'right');}`,
    `function drawSlide1StoryTitle(content){const x=${WEEKLY_SLIDE1_DAILY_STORY_GRID.x},y=${WEEKLY_SLIDE1_DAILY_STORY_GRID.title.y},w=${WEEKLY_SLIDE1_DAILY_STORY_GRID.width},h=${WEEKLY_SLIDE1_DAILY_STORY_GRID.title.height},title=normalizeText(content.title),titleSize=fittedSize(title,w-110,52,34,800);box(x,y,w,h);plainText(title,x+55,y+88,titleSize,800,slide1Ink,'left');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=3;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x+58,y+116);ctx.lineTo(x+110,y+116);ctx.stroke();ctx.restore();}`,
    `function weeklyStoryBoxLayout(content,w){const settings=${JSON.stringify(LOKA_WEEKLY_STORY_ADAPTIVE_LAYOUT)},visual=content.synthesis,primaryLines=Math.max(1,wrappedLines(visual.primaryLine,w-${LOKA_EDITORIAL_SUMMARY_FRAME.insetX * 2},34,${LOKA_EDITORIAL_SUMMARY_FRAME.primary.weight}).length),secondaryLines=Math.max(1,Math.min(${LOKA_EDITORIAL_SUMMARY_FRAME.secondary.maximumLines},wrappedLines(visual.secondaryLine,w-${LOKA_EDITORIAL_SUMMARY_FRAME.insetX * 2},25,${LOKA_EDITORIAL_SUMMARY_FRAME.secondary.weight}).length)),available=settings.bottom-settings.top-settings.firstGap-settings.secondGap,summaryWanted=Math.max(settings.summary.preferredHeight,150+primaryLines*38+secondaryLines*34),summaryHeight=Math.min(355,summaryWanted),stripHeight=settings.strip.preferredHeight,factsHeight=available-summaryHeight-stripHeight;if(factsHeight<settings.facts.minimumHeight){const shortage=settings.facts.minimumHeight-factsHeight;summaryHeight=Math.max(settings.summary.minimumHeight,summaryHeight-shortage);factsHeight=available-summaryHeight-stripHeight;}if(factsHeight<settings.facts.minimumHeight||summaryHeight<settings.summary.minimumHeight||stripHeight<settings.strip.minimumHeight)throw new Error('weekly_story_content_does_not_fit');const facts={y:settings.top,height:factsHeight},summary={y:settings.top+factsHeight+settings.firstGap,height:summaryHeight},strip={y:settings.top+factsHeight+settings.firstGap+summaryHeight+settings.secondGap,height:stripHeight};return{facts,summary,strip};}`,
    "function drawStoryTemperatureFact(fact,icon,x,y,w,h,iconWidth,iconHeight){const offset=(h-704)/2,labelSize=fittedSize(fact.label,w-40,21,16,700),dateSize=fittedSize(fact.dateLabel,w-40,34,20,600);trackedText(fact.label,x+w/2,y+139+offset,labelSize,700,slide1Ink,.35,'center');drawFactAccent(x+w/2,y+163+offset);drawImageCentered(icon,x+w/2,y+297+offset,iconWidth,iconHeight);plainText(fact.dateLabel,x+w/2,y+462+offset,dateSize,600,slide1Ink,'center');plainText(fact.temperatureLabel,x+w/2,y+577+offset,fittedSize(fact.temperatureLabel,w-24,82,62,700),700,slide1Ink,'center');}",
    "function drawStoryDaylightFact(content,icon,x,y,w,h){const offset=(h-704)/2,labelSize=fittedSize(content.label,w-40,21,16,700),periodSize=fittedSize(content.periodLabel,w-40,33,19,600),deltaValue=daylightDeltaValue(content.deltaLabel);trackedText(content.label,x+w/2,y+139+offset,labelSize,700,slide1Ink,.35,'center');drawFactAccent(x+w/2,y+163+offset);drawImageCentered(icon,x+w/2,y+297+offset,220,170);plainText(content.periodLabel,x+w/2,y+462+offset,periodSize,600,slide1Ink,'center');plainText(deltaValue,x+w/2,y+577+offset,fittedSize(deltaValue,w-24,82,60,700),700,slide1Ink,'center');}",
    `function drawSlide1StoryFacts(content,dailyIcons,daylightIcon,thermometerIcon,layout){const x=${WEEKLY_SLIDE1_DAILY_STORY_GRID.x},y=layout.y,w=${WEEKLY_SLIDE1_DAILY_STORY_GRID.width},h=layout.height,columnWidth=w/3,hotIcon=dailyIcons[content.hottestDay.dayIndex];box(x,y,w,h);ctx.save();ctx.strokeStyle='${LOKA_PUBLICATION_STYLE.separator.color}';ctx.lineWidth=${LOKA_PUBLICATION_STYLE.separator.width};for(let index=1;index<3;index++){const separatorX=x+columnWidth*index;ctx.beginPath();ctx.moveTo(separatorX,y+24);ctx.lineTo(separatorX,y+h-24);ctx.stroke();}ctx.restore();drawStoryTemperatureFact(content.coldestMorning,thermometerIcon,x,y,columnWidth,h,175,160);drawStoryTemperatureFact(content.hottestDay,hotIcon,x+columnWidth,y,columnWidth,h,220,185);drawStoryDaylightFact(content.daylight,daylightIcon,x+columnWidth*2,y,columnWidth,h);}`,
    `function drawSlide1StorySummary(content,layout){const x=${WEEKLY_SLIDE1_DAILY_STORY_GRID.x},y=layout.y,w=${WEEKLY_SLIDE1_DAILY_STORY_GRID.width},h=layout.height,visual=content.synthesis,left=x+${LOKA_EDITORIAL_SUMMARY_FRAME.insetX},maxWidth=w-${LOKA_EDITORIAL_SUMMARY_FRAME.insetX * 2},primary=normalizeText(visual.primaryLine),secondary=normalizeText(visual.secondaryLine),primarySize=fittedSize(primary,maxWidth,34,23,${LOKA_EDITORIAL_SUMMARY_FRAME.primary.weight}),secondaryFit=fitSlide1EditorialLines(secondary,maxWidth,25,19,${LOKA_EDITORIAL_SUMMARY_FRAME.secondary.weight},visual.secondaryMaximumLines),secondaryLineHeight=Math.round(secondaryFit.size*${LOKA_EDITORIAL_SUMMARY_FRAME.secondaryLineHeightRatio}),secondaryHeight=secondaryFit.size+Math.max(0,secondaryFit.lines.length-1)*secondaryLineHeight,groupHeight=primarySize+${LOKA_EDITORIAL_SUMMARY_FRAME.accent.offsetBelowPrimary}+${LOKA_EDITORIAL_SUMMARY_FRAME.accent.lineWidth}+${LOKA_EDITORIAL_SUMMARY_FRAME.gapAfterAccent}+secondaryHeight,top=y+(h-groupHeight)/2,primaryBaseline=top+primarySize,accentY=primaryBaseline+${LOKA_EDITORIAL_SUMMARY_FRAME.accent.offsetBelowPrimary};slide1SummaryBox(x,y,w,h);plainText(primary,left,primaryBaseline,primarySize,${LOKA_EDITORIAL_SUMMARY_FRAME.primary.weight},slide1Ink,'left');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=${LOKA_EDITORIAL_SUMMARY_FRAME.accent.lineWidth};ctx.lineCap='round';ctx.beginPath();ctx.moveTo(left,accentY);ctx.lineTo(left+58,accentY);ctx.stroke();ctx.restore();const firstSecondaryBaseline=accentY+${LOKA_EDITORIAL_SUMMARY_FRAME.gapAfterAccent}+secondaryFit.size;secondaryFit.lines.forEach((line,index)=>plainText(line,left,firstSecondaryBaseline+index*secondaryLineHeight,secondaryFit.size,${LOKA_EDITORIAL_SUMMARY_FRAME.secondary.weight},rgba(slide1Ink,.99),'left'));}`,
    `function drawSlide1StoryDayStrip(days,dailyIcons,content,layout){const x=${WEEKLY_SLIDE1_DAILY_STORY_GRID.x},y=layout.y,w=${WEEKLY_SLIDE1_DAILY_STORY_GRID.width},h=layout.height,columnWidth=w/days.length,coldDayIndexes=new Set(factMatches(content.coldestMorning).map(match=>match.dayIndex)),hotDayIndexes=new Set(factMatches(content.hottestDay).map(match=>match.dayIndex));slide1StripBox(x,y,w,h);ctx.save();ctx.strokeStyle='${LOKA_PUBLICATION_STYLE.separator.color}';ctx.lineWidth=${LOKA_PUBLICATION_STYLE.separator.width};for(let index=1;index<days.length;index++){const separatorX=Math.round(x+columnWidth*index)+.5;ctx.beginPath();ctx.moveTo(separatorX,y+18);ctx.lineTo(separatorX,y+h-18);ctx.stroke();}ctx.restore();days.forEach(function(day,index){if(coldDayIndexes.has(day.dayIndex))drawSlide1DayMarker('COLD',x+columnWidth*index,y,columnWidth,h);if(hotDayIndexes.has(day.dayIndex))drawSlide1DayMarker('HOT',x+columnWidth*index,y,columnWidth,h);});days.forEach(function(day,index){const centerX=x+columnWidth*(index+.5),icon=dailyIcons[index];plainText(day.weekdayLabel,centerX,y+42,17,700,slide1Ink,'center');plainText(day.dayLabel,centerX,y+70,19,560,slide1Ink,'center');drawImageCentered(icon,centerX,y+150,106,82);const minLabel=String(Math.round(day.minTemperatureC))+'°',maxLabel=String(Math.round(day.maxTemperatureC))+'°',temperatureY=y+h-34;plainText(minLabel,centerX-7,temperatureY,24,600,slide1Ink,'right');plainText('/',centerX,temperatureY,17,450,slide1Ink,'center');plainText(maxLabel,centerX+7,temperatureY,24,600,slide1Ink,'left');});}`,
    `function drawSlide1StorySignature(){plainText(model.signature||model.brand.slogan,${WEEKLY_SLIDE1_DAILY_STORY_GRID.signature.x},${WEEKLY_SLIDE1_DAILY_STORY_GRID.signature.baseline},${WEEKLY_SLIDE1_DAILY_STORY_GRID.signature.size},${WEEKLY_SLIDE1_DAILY_STORY_GRID.signature.weight},slide1Ink,'center');ctx.save();ctx.strokeStyle=gold;ctx.lineWidth=${WEEKLY_SLIDE1_DAILY_STORY_GRID.signature.underlineWidth};ctx.lineCap='round';ctx.beginPath();ctx.moveTo(${WEEKLY_SLIDE1_DAILY_STORY_GRID.signature.underlineStartX},${WEEKLY_SLIDE1_DAILY_STORY_GRID.signature.underlineY});ctx.lineTo(${WEEKLY_SLIDE1_DAILY_STORY_GRID.signature.underlineEndX},${WEEKLY_SLIDE1_DAILY_STORY_GRID.signature.underlineY});ctx.stroke();ctx.restore();}`,
    "function drawSlide1Story(canvas,slide,background,logo,dailyIcons,daylightIcon,thermometerIcon){ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height);cover(background,canvas.width,canvas.height);const layout=weeklyStoryBoxLayout(plan.slide1,canvas.width);drawSlide1StoryHeader(logo);drawSlide1StoryTitle(plan.overviewTitle);drawSlide1StoryFacts(plan.slide1,dailyIcons,daylightIcon,thermometerIcon,layout.facts);drawSlide1StorySummary(plan.slide1,layout.summary);drawSlide1StoryDayStrip(plan.slide1.dailyStrip,dailyIcons,plan.slide1,layout.strip);drawSlide1StorySignature();}",
