...
'<style>:root{--ink:#12264a;--gold:#fdb515;--paper:#f3f1eb;--muted:#6f716f;--dark:#171715}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif}.wrap{max-width:1180px;margin:0 auto;padding:24px 18px 48px}.toolbar{background:#fff;border-radius:24px;padding:20px 22px;margin-bottom:22px;box-shadow:0 12px 40px rgba(18,38,74,.08)}.topline{display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{font-size:27px;font-weight:850;letter-spacing:.06em}.badge{font-size:11px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}.toolbar h1{font-size:28px;line-height:1.05;margin:20px 0 6px}.muted{font-size:13px;color:var(--muted)}.carousel{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:22px}.slide-card,.story-card{background:#fff;border-radius:24px;padding:14px;box-shadow:0 14px 46px rgba(18,38,74,.1)}.slide-head{display:flex;justify-content:space-between;gap:8px;padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.canvas-wrap{overflow:hidden;border-radius:18px;background:#d6d4cf}.canvas-wrap canvas{display:block;width:100%;height:auto}.activity-list{display:flex;flex-direction:column;gap:7px;margin:12px 2px 0}.activity{border-radius:11px;padding:8px 10px;font-size:12px;line-height:1.35}.activity.favorable{background:#edf7ef;color:#21613b}.activity.mixed{background:#faf3e3;color:#7a5b16}.activity.unfavorable{background:#f9e9e7;color:#8c302b}.secondary{width:100%;margin-top:12px;border:0;border-radius:12px;padding:12px 10px;background:#f0f0ed;color:var(--dark);font:650 12px/1 -apple-system,BlinkMacSystemFont,sans-serif;cursor:pointer}.story-card{max-width:420px;margin:28px auto 0}.story-head{padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.note{text-align:center;color:var(--muted);font-size:11px;line-height:1.5;margin:22px auto 0;max-width:760px}</style></head><body><main class="wrap"><section class="toolbar"><div class="topline"><div class="brand">LOKA!</div><div class="badge">La semaine à Tarnos · V24 · aperçu</div></div><h1>' + escapeHtml(plan.story.relay.title) + '</h1><div class="muted">' + escapeHtml(range) + ' · ' + String(plan.slides.length) + ' slide' + (plan.slides.length > 1 ? 's' : '') + '</div></section><section class="carousel" aria-label="Carrousel hebdomadaire">' + cards + '</section><section class="story-card"><div class="story-head">STORY · RELAIS DE LA PUBLICATION</div><div class="canvas-wrap"><canvas id="story-relay" width="' + String(WEEKLY_STORY_WIDTH) + '" height="' + String(WEEKLY_STORY_HEIGHT) + '" aria-label="' + escapeHtml(storyLabel) + '"></canvas></div><button class="secondary" id="download-story" type="button">Télécharger le relais Story</button></section><p class="note">Le rendu utilise le logo LOKA!, les box translucides, les pictogrammes redessinés et les fonds des 24 scènes. La Story reste un relais du carrousel ; elle ne constitue pas un bulletin autonome.</p></main><script>';
  ...
  const script = [
    ...
    "function drawOverviewTitle(content){const title=normalizeText(content.title),subtitle=normalizeText(content.subtitle);const titleSize=fittedSize(title,870,68,46,820),subtitleSize=fittedSize(subtitle,870,36,24,520);text(title,105,286,titleSize,820,ink,'left');ctx.save();ctx.strokeStyle=gold;ctx.lineWidth=5;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(108,310);ctx.lineTo(188,310);ctx.stroke();ctx.restore();text(subtitle,108,356,subtitleSize,520,ink,'left');}",
    ...
    "function slide1StripBox(x,y,w,h){ctx.save();rr(x,y,w,h,32);ctx.fillStyle='rgba(255,255,255,.64)';ctx.fill();ctx.strokeStyle='rgba(255,255,255,.96)';ctx.lineWidth=1.5;ctx.stroke();ctx.restore();}",
    "function slide1SummaryBox(x,y,w,h){ctx.save();rr(x,y,w,h,30);ctx.fillStyle='rgba(255,255,255,.48)';ctx.fill();ctx.strokeStyle='rgba(255,255,255,.94)';ctx.lineWidth=1.45;ctx.stroke();ctx.restore();}",
    "function drawSlide1TemperatureFact(fact,icon,x,y,w,accent){slide1MetricBox(x,y,w,380);trackedText(fact.label,x+w/2,y+67,18,760,ink,.5,'center');drawImageCentered(icon,x+w/2,y+160,140,120);text(fact.dateLabel,x+w/2,y+264,30,720,ink,'center');text(fact.temperatureLabel,x+w/2,y+346,fittedSize(fact.temperatureLabel,w-28,78,58,800),800,accent,'center');}",
    "function drawSlide1DaylightFact(content,daylightIcon,x,y,w){slide1MetricBox(x,y,w,380);trackedText(content.label,x+w/2,y+67,18,760,ink,.5,'center');drawImageCentered(daylightIcon,x+w/2,y+164,150,112);text(content.deltaLabel,x+w/2,y+278,fittedSize(content.deltaLabel,w-28,42,30,820),820,gold,'center');const range='Lever '+content.start.sunriseLabel+' → '+content.end.sunriseLabel+' · Coucher '+content.start.sunsetLabel+' → '+content.end.sunsetLabel;text(range,x+w/2,y+334,fittedSize(range,w-26,15,11,650),650,ink,'center');}",
    ...
    "function drawSlide1Summary(content){slide1SummaryBox(45,820,990,105);completeWrap(content.synthesis.text,540,883,900,35,29,20,520,ink,'center',content.synthesis.maximumLines);}",
    "function drawSlide1DayMarker(kind,x,y,w,h){const accent=kind==='HOT'?gold:'#4b8fc5';ctx.save();rr(x+4,y+4,w-8,h-8,18);ctx.fillStyle=kind==='HOT'?'rgba(253,181,21,.13)':'rgba(75,143,197,.13)';ctx.fill();ctx.strokeStyle=accent;ctx.lineWidth=3;ctx.stroke();ctx.restore();}",
    "function drawSlide1DayStrip(days,dailyIcons,content){const x=45,y=945,w=990,h=250,columnWidth=w/days.length;days.forEach(function(day,index){if(day.dayIndex===content.coldestMorning.dayIndex)drawSlide1DayMarker('COLD',x+columnWidth*index,y,columnWidth,h);if(day.dayIndex===content.hottestDay.dayIndex)drawSlide1DayMarker('HOT',x+columnWidth*index,y,columnWidth,h);});ctx.save();ctx.strokeStyle='rgba(18,38,74,.2)';ctx.lineWidth=1.15;for(let index=1;index<days.length;index++){const separatorX=x+columnWidth*index;ctx.beginPath();ctx.moveTo(separatorX,y+18);ctx.lineTo(separatorX,y+h-18);ctx.stroke();}ctx.restore();days.forEach(function(day,index){const centerX=x+columnWidth*(index+.5),icon=dailyIcons[index];text(day.weekdayLabel,centerX,y+43,17,800,ink,'center');text(day.dayLabel,centerX,y+71,19,760,ink,'center');drawImageCentered(icon,centerX,y+143,106,82);const minLabel=String(Math.round(day.minTemperatureC))+'°',maxLabel=String(Math.round(day.maxTemperatureC))+'°';text(minLabel,centerX-7,y+220,24,800,ink,'right');text('/',centerX,y+220,17,680,ink,'center');text(maxLabel,centerX+7,y+220,24,800,gold,'left');});}",
    "function drawSlide1Overview(canvas,slide,background,logo,dailyIcons,daylightIcon,thermometerIcon){ctx=canvas.getContext('2d');const width=canvas.width,height=canvas.height;ctx.clearRect(0,0,width,height);cover(background,width,height);overlay(width,height);drawHeader(logo,plan.headerDateLabel,width);box(45,185,990,210);drawOverviewTitle(plan.overviewTitle);drawSlide1Facts(plan.slide1,dailyIcons,daylightIcon,thermometerIcon);drawSlide1Summary(plan.slide1);slide1StripBox(45,945,990,250);drawSlide1DayStrip(plan.slide1.dailyStrip,dailyIcons,plan.slide1);drawSignature(1274,'rgba(255,255,255,.94)');}",
