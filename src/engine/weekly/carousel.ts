...
/**
 * Locked editorial architecture for every contextual slide: title, information,
 * editorial phrase. Only content may vary. Its proportions deliberately reuse
 * slide 1's vertical rhythm while the shared header and signature stay fixed.
 */
export const WEEKLY_COMPLEMENTARY_BOX_LAYOUT = {
  content: { y: 336, height: 690, bottom: 1026 },
  editorial: { y: 1056, height: 175, bottom: 1231 }
} as const;
/**
...
'<style>:root{--ink:#12264a;--gold:#fdb515;--paper:#f3f1eb;--muted:#6f716f;--dark:#171715}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif}.wrap{max-width:1180px;margin:0 auto;padding:24px 18px 48px}.toolbar{background:#fff;border-radius:24px;padding:20px 22px;margin-bottom:22px;box-shadow:0 12px 40px rgba(18,38,74,.08)}.topline{display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{font-size:27px;font-weight:850;letter-spacing:.06em}.badge{font-size:11px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}.toolbar h1{font-size:28px;line-height:1.05;margin:20px 0 6px}.muted{font-size:13px;color:var(--muted)}.pilot{display:flex;justify-content:space-between;gap:14px;margin:16px 0 0;padding:12px 14px;border-radius:14px;font-size:12px;line-height:1.35}.pilot strong,.pilot span{display:block}.pilot span{margin-top:3px}.pilot small{align-self:center;white-space:nowrap}.pilot.pass{background:#edf7ef;color:#21613b}.pilot.review{background:#faf3e3;color:#7a5b16}.pilot.blocked{background:#f9e9e7;color:#8c302b}.carousel{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:22px}.slide-card,.story-card{background:#fff;border-radius:24px;padding:14px;box-shadow:0 14px 46px rgba(18,38,74,.1)}.slide-head{display:flex;justify-content:space-between;gap:8px;padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.canvas-wrap{overflow:hidden;border-radius:18px;background:#d6d4cf}.canvas-wrap canvas{display:block;width:100%;height:auto}.activity-list{display:flex;flex-direction:column;gap:7px;margin:12px 2px 0}.activity{border-radius:11px;padding:8px 10px;font-size:12px;line-height:1.35}.activity.favorable{background:#edf7ef;color:#21613b}.activity.mixed{background:#faf3e3;color:#7a5b16}.activity.unfavorable{background:#f9e9e7;color:#8c302b}.secondary{width:100%;margin-top:12px;border:0;border-radius:12px;padding:12px 10px;background:#f0f0ed;color:var(--dark);font:650 12px/1 -apple-system,BlinkMacSystemFont,sans-serif;cursor:pointer}.story-card{max-width:420px;margin:28px auto 0}.story-head{padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.note{text-align:center;color:var(--muted);font-size:11px;line-height:1.5;margin:22px auto 0;max-width:760px}</style></head><body><main class="wrap"><section class="toolbar"><div class="topline"><div class="brand">LOKA!</div><div class="badge">' + escapeHtml(previewBadge) + '</div></div><h1>' + escapeHtml(previewTitle) + '</h1><div class="muted">' + escapeHtml(range) + ' · ' + String(plan.slides.length) + ' slide' + (plan.slides.length > 1 ? 's' : '') + '</div>' + pilotPanel + '</section>' + carouselSection + storySection + '<p class="note">' + escapeHtml(previewNote) + '</p></main><script>';
  ...
  const script = [
    ...
    `function drawSlide1TemperatureFact(fact,icon,x,y,w,iconWidth,iconHeight){slide1MetricBox(x,y,w,${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.height});const labelSize=fittedSize(fact.label,w-44,19,15,700),dateSize=fittedSize(fact.dateLabel,w-44,30,18,560);trackedText(fact.label,x+w/2,y+92,labelSize,700,slide1Ink,.35,'center');drawImageCentered(icon,x+w/2,y+200,iconWidth,iconHeight);plainText(fact.dateLabel,x+w/2,y+329,dateSize,560,slide1Ink,'center');plainText(fact.temperatureLabel,x+w/2,y+430,fittedSize(fact.temperatureLabel,w-28,70,56,700),700,slide1Ink,'center');}`,
    ...
    `function drawSlide1DaylightFact(content,daylightIcon,x,y,w){slide1MetricBox(x,y,w,${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.height});const labelSize=fittedSize(content.label,w-44,19,15,700),periodSize=fittedSize(content.periodLabel,w-44,30,18,560),deltaValue=daylightDeltaValue(content.deltaLabel);trackedText(content.label,x+w/2,y+92,labelSize,700,slide1Ink,.35,'center');drawImageCentered(daylightIcon,x+w/2,y+200,205,154);plainText(content.periodLabel,x+w/2,y+329,periodSize,560,slide1Ink,'center');plainText(deltaValue,x+w/2,y+430,fittedSize(deltaValue,w-28,70,56,700),700,slide1Ink,'center');}`,
    ...
    `function drawComparisonItem(item,cx,valueY,labelY){plainText(item.value,cx,valueY,fittedSize(item.value,390,48,35,700),700,slide1Ink,'center');completePlainWrap(item.label,cx,labelY,390,23,19,17,650,slide1Ink,'center',2);}`,
    `function drawComplementaryContentBox(content,icon,x,y,w,h){const visual=content.presentation;drawImageCentered(icon,x+w/2,y+145,155,140);plainText(visual.headline,x+w/2,y+280,fittedSize(visual.headline,w-110,110,62,720),720,slide1Ink,'center');completePlainWrap(visual.subtitle,x+w/2,y+340,w-100,30,27,22,700,slide1Ink,'center',2);ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=${LOKA_EDITORIAL_SUMMARY_FRAME.accent.lineWidth};ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x+w/2-${LOKA_EDITORIAL_SUMMARY_FRAME.accent.width / 2},y+376);ctx.lineTo(x+w/2+${LOKA_EDITORIAL_SUMMARY_FRAME.accent.width / 2},y+376);ctx.stroke();ctx.restore();if(visual.comparison){const leftX=x+w*.28,rightX=x+w*.72;drawComparisonItem(visual.comparison.left,leftX,y+452,y+497);drawComparisonItem(visual.comparison.right,rightX,y+452,y+497);ctx.save();ctx.strokeStyle='rgba(6,31,76,.23)';ctx.lineWidth=1.25;ctx.beginPath();ctx.moveTo(x+w/2,y+414);ctx.lineTo(x+w/2,y+500);ctx.stroke();ctx.restore();return;}trackedText('REPÈRE',x+w/2,y+430,17,700,slide1Ink,2.2,'center');completePlainWrap(content.secondaryLine,x+w/2,y+492,w-150,32,24,18,560,rgba(slide1Ink,.96),'center',2);}`,
    `function drawComplementaryEditorialBox(content,x,y,w,h){const visual=content.presentation,left=x+${LOKA_EDITORIAL_SUMMARY_FRAME.insetX},maxWidth=w-${LOKA_EDITORIAL_SUMMARY_FRAME.insetX * 2},fit=fitSlide1EditorialLines(visual.editorialLine,maxWidth,${LOKA_EDITORIAL_SUMMARY_FRAME.primary.maximumSize},${LOKA_EDITORIAL_SUMMARY_FRAME.primary.minimumSize},${LOKA_EDITORIAL_SUMMARY_FRAME.primary.weight},2),lineHeight=Math.round(fit.size*${LOKA_EDITORIAL_SUMMARY_FRAME.secondaryLineHeightRatio}),textHeight=fit.size+Math.max(0,fit.lines.length-1)*lineHeight,groupHeight=${LOKA_EDITORIAL_SUMMARY_FRAME.accent.lineWidth}+${LOKA_EDITORIAL_SUMMARY_FRAME.gapAfterAccent}+textHeight,accentY=y+(h-groupHeight)/2;ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=${LOKA_EDITORIAL_SUMMARY_FRAME.accent.lineWidth};ctx.lineCap='round';ctx.beginPath();ctx.moveTo(left,accentY);ctx.lineTo(left+${LOKA_EDITORIAL_SUMMARY_FRAME.accent.width},accentY);ctx.stroke();ctx.restore();const firstBaseline=accentY+${LOKA_EDITORIAL_SUMMARY_FRAME.accent.lineWidth}+${LOKA_EDITORIAL_SUMMARY_FRAME.gapAfterAccent}+fit.size;ctx.save();font(fit.size,${LOKA_EDITORIAL_SUMMARY_FRAME.primary.weight});fit.lines.forEach((line,index)=>drawPlainTextLine(line,left,firstBaseline+index*lineHeight,slide1Ink,'left'));ctx.restore();}`,
    `function drawComplementarySlide(canvas,slide,background,logo,icon){const content=slide.complementary,x=${WEEKLY_COMPLEMENTARY_DAILY_FEED_GRID.x},w=${WEEKLY_COMPLEMENTARY_DAILY_FEED_GRID.width},contentBox=${JSON.stringify(WEEKLY_COMPLEMENTARY_BOX_LAYOUT.content)},editorial=${JSON.stringify(WEEKLY_COMPLEMENTARY_BOX_LAYOUT.editorial)};if(!content||content.frame!=='WEEKLY_SHARED_V1'||!content.presentation)throw new Error('weekly_shared_frame_required');drawWeeklySharedFrame(canvas,background,logo,content);slide1MetricBox(x,contentBox.y,w,contentBox.height);slide1SummaryBox(x,editorial.y,w,editorial.height);drawComplementaryContentBox(content,icon,x,contentBox.y,w,contentBox.height);drawComplementaryEditorialBox(content,x,editorial.y,w,editorial.height);drawSlide1FeedSignature();}`,
