...
'<style>:root{--ink:#12264a;--gold:#fdb515;--paper:#f3f1eb;--muted:#6f716f;--dark:#171715}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif}.wrap{max-width:1180px;margin:0 auto;padding:24px 18px 48px}.toolbar{background:#fff;border-radius:24px;padding:20px 22px;margin-bottom:22px;box-shadow:0 12px 40px rgba(18,38,74,.08)}.topline{display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{font-size:27px;font-weight:850;letter-spacing:.06em}.badge{font-size:11px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}.toolbar h1{font-size:28px;line-height:1.05;margin:20px 0 6px}.muted{font-size:13px;color:var(--muted)}.pilot{display:flex;justify-content:space-between;gap:14px;margin:16px 0 0;padding:12px 14px;border-radius:14px;font-size:12px;line-height:1.35}.pilot strong,.pilot span{display:block}.pilot span{margin-top:3px}.pilot small{align-self:center;white-space:nowrap}.pilot.pass{background:#edf7ef;color:#21613b}.pilot.review{background:#faf3e3;color:#7a5b16}.pilot.blocked{background:#f9e9e7;color:#8c302b}.carousel{display:grid;gap:22px}.slide-card{background:#fff;border-radius:24px;padding:14px;box-shadow:0 14px 46px rgba(18,38,74,.1)}.slide-head{display:flex;justify-content:space-between;gap:8px;padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.format-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:18px}.format-panel{min-width:0}.format-label{padding:0 4px 9px;font-size:10px;font-weight:800;letter-spacing:.1em;color:var(--muted)}.canvas-wrap{overflow:hidden;border-radius:18px;background:#d6d4cf}.canvas-wrap canvas{display:block;width:100%;height:auto}.story-wrap{max-width:420px;margin:0 auto}.activity-list{display:flex;flex-direction:column;gap:7px;margin:12px 2px 0}.activity{border-radius:11px;padding:8px 10px;font-size:12px;line-height:1.35}.activity.favorable{background:#edf7ef;color:#21613b}.activity.mixed{background:#faf3e3;color:#7a5b16}.activity.unfavorable{background:#f9e9e7;color:#8c302b}.secondary{width:100%;margin-top:12px;border:0;border-radius:12px;padding:12px 10px;background:#f0f0ed;color:var(--dark);font:650 12px/1 -apple-system,BlinkMacSystemFont,sans-serif;cursor:pointer}.note{text-align:center;color:var(--muted);font-size:11px;line-height:1.5;margin:22px auto 0;max-width:760px}</style></head><body><main class="wrap"><section class="toolbar"><div class="topline"><div class="brand">LOKA!</div><div class="badge">' + escapeHtml(previewBadge) + '</div></div><h1>' + escapeHtml(previewTitle) + '</h1><div class="muted">' + escapeHtml(range) + ' · ' + String(plan.slides.length) + ' slide' + (plan.slides.length > 1 ? 's' : '') + '</div>' + pilotPanel + '</section>' + carouselSection + storySection + '<p class="note">' + escapeHtml(previewNote) + '</p></main><script>';
  ...
  const script = [
    ...
    `function drawSlide1Header(logo,date,width,layout=FEED_LAYOUT){const s=layout.visualScale,h=WEEKLY_HIERARCHY;drawLokaLogo(logo,${LOKA_DAILY_FEED_FRAME.header.logoX},layoutY(${LOKA_DAILY_FEED_FRAME.header.logoCenterY},layout),${LOKA_DAILY_FEED_FRAME.header.logoWidth}*s*h.logo,${LOKA_DAILY_FEED_FRAME.header.logoHeight}*s*h.logo);trackedText(String(plan.citySlug==='tarnos'?'TARNOS':plan.citySlug).toUpperCase(),${LOKA_DAILY_FEED_FRAME.header.cityX},layoutY(${LOKA_DAILY_FEED_FRAME.header.cityBaseline},layout),${LOKA_DAILY_FEED_FRAME.header.citySize}*s*h.header,760,ink,${LOKA_DAILY_FEED_FRAME.header.cityTracking},'center');text(normalizeText(date),width-50,layoutY(${LOKA_DAILY_FEED_FRAME.header.dateBaseline},layout),fittedSize(date,320,${LOKA_DAILY_FEED_FRAME.header.dateSize}*s*h.header,${LOKA_DAILY_FEED_FRAME.header.dateSize}*s,700),700,ink,'right');}`,
    ...
    `function drawOverviewTitle(content,layout=FEED_LAYOUT){const s=layout.visualScale,title=normalizeText(content.title),titleSize=fittedSize(title,870,48*s*WEEKLY_HIERARCHY.title,32*s,820);text(title,${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 55},layoutY(241,layout),titleSize,820,ink,'left');ctx.save();ctx.strokeStyle=gold;ctx.lineWidth=3;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 58},layoutY(263,layout));ctx.lineTo(${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 110},layoutY(263,layout));ctx.stroke();ctx.restore();}`,
    ...
    `function drawSlide1TemperatureFact(fact,icon,x,y,w,iconWidth,iconHeight,layout=FEED_LAYOUT){const s=layout.visualScale,h=WEEKLY_HIERARCHY,labelSize=fittedSize(fact.label,w-34,19*s*h.section,15*s,770),dateSize=fittedSize(fact.dateLabel,w-34,30*s*h.factDate,18*s,650);trackedText(fact.label,x+w/2,layoutY(y+72,layout),labelSize,770,slide1Ink,.35,'center');drawFactAccent(x+w/2,layoutY(y+92,layout));drawImageCentered(icon,x+w/2,layoutY(y+205,layout),iconWidth*s*h.pictogram,iconHeight*s*h.pictogram);plainText(fact.dateLabel,x+w/2,layoutY(y+329,layout),dateSize,650,slide1Ink,'center');plainText(fact.temperatureLabel,x+w/2,layoutY(y+430,layout),fittedSize(fact.temperatureLabel,w-28,70*s*h.metric,56*s,780),780,slide1Ink,'center');}`,
    ...
    `function drawSlide1DaylightFact(content,daylightIcon,x,y,w,layout=FEED_LAYOUT){const s=layout.visualScale,h=WEEKLY_HIERARCHY,labelSize=fittedSize(content.label,w-34,19*s*h.section,15*s,770),periodSize=fittedSize(content.periodLabel,w-22,30*s*h.factDate,18*s,650),deltaValue=daylightDeltaValue(content.deltaLabel);trackedText(content.label,x+w/2,layoutY(y+72,layout),labelSize,770,slide1Ink,.35,'center');drawFactAccent(x+w/2,layoutY(y+92,layout));drawImageCentered(daylightIcon,x+w/2,layoutY(y+205,layout),205*s*h.pictogram,154*s*h.pictogram);plainText(content.periodLabel,x+w/2,layoutY(y+329,layout),periodSize,650,slide1Ink,'center');plainText(deltaValue,x+w/2,layoutY(y+430,layout),fittedSize(deltaValue,w-16,70*s*h.metric,56*s,780),780,slide1Ink,'center');}`,
    ...
    `function drawSlide1Summary(content,layout=FEED_LAYOUT){const x=${WEEKLY_SLIDE1_DAILY_FEED_GRID.x},y=layoutY(${WEEKLY_SLIDE1_DAILY_FEED_GRID.summary.y},layout),w=${WEEKLY_SLIDE1_DAILY_FEED_GRID.width},h=layoutH(${WEEKLY_SLIDE1_DAILY_FEED_GRID.summary.height},layout),s=layout.visualScale,visual=content.synthesis,main=normalizeText(visual.primaryLine),secondary=normalizeText(visual.secondaryLine),left=x+${LOKA_EDITORIAL_SUMMARY_FRAME.insetX},maxWidth=w-${LOKA_EDITORIAL_SUMMARY_FRAME.insetX * 2},primaryWeight=720,secondaryWeight=650,primarySize=fittedSize(main,maxWidth,${LOKA_EDITORIAL_SUMMARY_FRAME.primary.maximumSize}*s*WEEKLY_HIERARCHY.editorialTitle,${LOKA_EDITORIAL_SUMMARY_FRAME.primary.minimumSize}*s,primaryWeight),secondaryFit=fitSlide1EditorialLines(secondary,maxWidth,${LOKA_EDITORIAL_SUMMARY_FRAME.secondary.maximumSize}*s*WEEKLY_HIERARCHY.editorialDetail,${LOKA_EDITORIAL_SUMMARY_FRAME.secondary.minimumSize}*s,secondaryWeight,visual.secondaryMaximumLines),secondaryLineHeight=Math.round(secondaryFit.size*${LOKA_EDITORIAL_SUMMARY_FRAME.secondaryLineHeightRatio}),secondaryHeight=secondaryFit.size+Math.max(0,secondaryFit.lines.length-1)*secondaryLineHeight,groupHeight=primarySize+${LOKA_EDITORIAL_SUMMARY_FRAME.accent.offsetBelowPrimary}+${LOKA_EDITORIAL_SUMMARY_FRAME.accent.lineWidth}+${LOKA_EDITORIAL_SUMMARY_FRAME.gapAfterAccent}+secondaryHeight,top=y+(h-groupHeight)/2,primaryBaseline=top+primarySize,accentY=primaryBaseline+${LOKA_EDITORIAL_SUMMARY_FRAME.accent.offsetBelowPrimary};if(groupHeight>h-24)throw new Error('weekly_slide1_editorial_vertical_overflow');slide1SummaryBox(x,y,w,h);text(main,left,primaryBaseline,primarySize,primaryWeight,slide1Ink,'left');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=${LOKA_EDITORIAL_SUMMARY_FRAME.accent.lineWidth};ctx.lineCap='round';ctx.beginPath();ctx.moveTo(left,accentY);ctx.lineTo(left+${LOKA_EDITORIAL_SUMMARY_FRAME.accent.width},accentY);ctx.stroke();ctx.restore();const firstSecondaryBaseline=accentY+${LOKA_EDITORIAL_SUMMARY_FRAME.gapAfterAccent}+secondaryFit.size;secondaryFit.lines.forEach((line,index)=>text(line,left,firstSecondaryBaseline+index*secondaryLineHeight,secondaryFit.size,secondaryWeight,rgba(slide1Ink,.99),'left'));}`,
    ...
    `function drawSlide1DayStrip(days,dailyIcons,content,layout=FEED_LAYOUT){const x=${WEEKLY_SLIDE1_DAILY_FEED_GRID.x},sourceY=${WEEKLY_SLIDE1_DAILY_FEED_GRID.dailyStrip.y},sourceH=${WEEKLY_SLIDE1_DAILY_FEED_GRID.dailyStrip.height},y=layoutY(sourceY,layout),w=${WEEKLY_SLIDE1_DAILY_FEED_GRID.width},h=layoutH(sourceH,layout),s=layout.visualScale,scale=WEEKLY_HIERARCHY,columnWidth=w/days.length,coldDayIndexes=new Set(factMatches(content.coldestMorning).map(match=>match.dayIndex)),hotDayIndexes=new Set(factMatches(content.hottestDay).map(match=>match.dayIndex));ctx.save();ctx.strokeStyle='rgba(6,31,76,.14)';ctx.lineWidth=1;for(let index=1;index<days.length;index++){const separatorX=Math.round(x+columnWidth*index)+.5;ctx.beginPath();ctx.moveTo(separatorX,layoutY(sourceY+18,layout));ctx.lineTo(separatorX,layoutY(sourceY+sourceH-18,layout));ctx.stroke();}ctx.restore();days.forEach(function(day,index){if(coldDayIndexes.has(day.dayIndex))drawSlide1DayMarker('COLD',x+columnWidth*index,y,columnWidth,h);if(hotDayIndexes.has(day.dayIndex))drawSlide1DayMarker('HOT',x+columnWidth*index,y,columnWidth,h);});days.forEach(function(day,index){const centerX=x+columnWidth*(index+.5),icon=dailyIcons[index],labelWidth=columnWidth-16,weekdaySize=fittedSize(day.weekdayLabel,labelWidth,17*s*scale.stripLabel,14*s,760);plainText(day.weekdayLabel,centerX,layoutY(sourceY+40,layout),weekdaySize,760,slide1Ink,'center');plainText(day.dayLabel,centerX,layoutY(sourceY+66,layout),19*s*scale.stripLabel,680,slide1Ink,'center');drawImageCentered(icon,centerX,layoutY(sourceY+133,layout),106*s*scale.stripIcon,82*s*scale.stripIcon);const minLabel=String(Math.round(day.minTemperatureC))+'°',maxLabel=String(Math.round(day.maxTemperatureC))+'°',tempSize=Math.min(fittedSize(minLabel,columnWidth/2-14,24*s*scale.stripValue,19*s,730),fittedSize(maxLabel,columnWidth/2-14,24*s*scale.stripValue,19*s,730));plainText(minLabel,centerX-7,layoutY(sourceY+210,layout),tempSize,730,slide1Ink,'right');plainText('/',centerX,layoutY(sourceY+210,layout),18*s,560,slide1Ink,'center');plainText(maxLabel,centerX+7,layoutY(sourceY+210,layout),tempSize,730,slide1Ink,'left');});}`,
    `function drawSlide1FeedSignature(layout=FEED_LAYOUT){const s=layout.visualScale;text(model.signature||model.brand.slogan,${WEEKLY_SLIDE1_DAILY_FEED_GRID.signature.x},layoutY(${WEEKLY_SLIDE1_DAILY_FEED_GRID.signature.baseline},layout),${WEEKLY_SLIDE1_DAILY_FEED_GRID.signature.size}*s*WEEKLY_HIERARCHY.signature,680,rgba(ink,${WEEKLY_SLIDE1_DAILY_FEED_GRID.signature.colorAlpha}),'center');ctx.save();ctx.strokeStyle=gold;ctx.lineWidth=${WEEKLY_SLIDE1_DAILY_FEED_GRID.signature.underlineWidth};ctx.lineCap='round';ctx.beginPath();ctx.moveTo(${WEEKLY_SLIDE1_DAILY_FEED_GRID.signature.underlineStartX},layoutY(${WEEKLY_SLIDE1_DAILY_FEED_GRID.signature.underlineY},layout));ctx.lineTo(${WEEKLY_SLIDE1_DAILY_FEED_GRID.signature.underlineEndX},layoutY(${WEEKLY_SLIDE1_DAILY_FEED_GRID.signature.underlineY},layout));ctx.stroke();ctx.restore();}`,
