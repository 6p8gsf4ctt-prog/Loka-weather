...
'<style>:root{--ink:#12264a;--gold:#fdb515;--paper:#f3f1eb;--muted:#6f716f;--dark:#171715}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif}.wrap{max-width:1180px;margin:0 auto;padding:24px 18px 48px}.toolbar{background:#fff;border-radius:24px;padding:20px 22px;margin-bottom:22px;box-shadow:0 12px 40px rgba(18,38,74,.08)}.topline{display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{font-size:27px;font-weight:850;letter-spacing:.06em}.badge{font-size:11px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}.toolbar h1{font-size:28px;line-height:1.05;margin:20px 0 6px}.muted{font-size:13px;color:var(--muted)}.pilot{display:flex;justify-content:space-between;gap:14px;margin:16px 0 0;padding:12px 14px;border-radius:14px;font-size:12px;line-height:1.35}.pilot strong,.pilot span{display:block}.pilot span{margin-top:3px}.pilot small{align-self:center;white-space:nowrap}.pilot.pass{background:#edf7ef;color:#21613b}.pilot.review{background:#faf3e3;color:#7a5b16}.pilot.blocked{background:#f9e9e7;color:#8c302b}.carousel{display:grid;gap:22px}.slide-card{background:#fff;border-radius:24px;padding:14px;box-shadow:0 14px 46px rgba(18,38,74,.1)}.slide-head{display:flex;justify-content:space-between;gap:8px;padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.format-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:18px}.format-panel{min-width:0}.format-label{padding:0 4px 9px;font-size:10px;font-weight:800;letter-spacing:.1em;color:var(--muted)}.canvas-wrap{overflow:hidden;border-radius:18px;background:#d6d4cf}.canvas-wrap canvas{display:block;width:100%;height:auto}.story-wrap{max-width:420px;margin:0 auto}.activity-list{display:flex;flex-direction:column;gap:7px;margin:12px 2px 0}.activity{border-radius:11px;padding:8px 10px;font-size:12px;line-height:1.35}.activity.favorable{background:#edf7ef;color:#21613b}.activity.mixed{background:#faf3e3;color:#7a5b16}.activity.unfavorable{background:#f9e9e7;color:#8c302b}.secondary{width:100%;margin-top:12px;border:0;border-radius:12px;padding:12px 10px;background:#f0f0ed;color:var(--dark);font:650 12px/1 -apple-system,BlinkMacSystemFont,sans-serif;cursor:pointer}.note{text-align:center;color:var(--muted);font-size:11px;line-height:1.5;margin:22px auto 0;max-width:760px}</style></head><body><main class="wrap"><section class="toolbar"><div class="topline"><div class="brand">LOKA!</div><div class="badge">' + escapeHtml(previewBadge) + '</div></div><h1>' + escapeHtml(previewTitle) + '</h1><div class="muted">' + escapeHtml(range) + ' · ' + String(plan.slides.length) + ' slide' + (plan.slides.length > 1 ? 's' : '') + '</div>' + pilotPanel + '</section>' + carouselSection + storySection + '<p class="note">' + escapeHtml(previewNote) + '</p></main><script>';
  ...
  const script = [
    ...
    "function drawSlide1StoryHeader(logo){drawLokaLogo(logo,50,104,190,64);trackedText(String(plan.citySlug==='tarnos'?'TARNOS':plan.citySlug).toUpperCase(),540,116,25,680,slide1Ink,8,'center');plainText(normalizeText(plan.headerDateFeed),1030,116,20,540,slide1Ink,'right');}",
    "function drawSlide1StoryTitle(content){const x=50,y=190,w=980,h=205,title=normalizeText(content.title),titleSize=fittedSize(title,w-110,68,50,760);box(x,y,w,h);plainText(title,x+55,y+112,titleSize,760,slide1Ink,'left');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=6;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x+58,y+145);ctx.lineTo(x+142,y+145);ctx.stroke();ctx.restore();}",
    "function drawStoryTemperatureFact(fact,icon,x,y,w,iconWidth,iconHeight){const labelSize=fittedSize(fact.label,w-40,21,16,700),dateSize=fittedSize(fact.dateLabel,w-40,34,20,600);trackedText(fact.label,x+w/2,y+72,labelSize,700,slide1Ink,.35,'center');drawFactAccent(x+w/2,y+96);drawImageCentered(icon,x+w/2,y+230,iconWidth,iconHeight);plainText(fact.dateLabel,x+w/2,y+395,dateSize,600,slide1Ink,'center');plainText(fact.temperatureLabel,x+w/2,y+510,fittedSize(fact.temperatureLabel,w-24,82,62,700),700,slide1Ink,'center');}",
    "function drawStoryDaylightFact(content,icon,x,y,w){const labelSize=fittedSize(content.label,w-40,21,16,700),periodSize=fittedSize(content.periodLabel,w-40,33,19,600),deltaValue=daylightDeltaValue(content.deltaLabel);trackedText(content.label,x+w/2,y+72,labelSize,700,slide1Ink,.35,'center');drawFactAccent(x+w/2,y+96);drawImageCentered(icon,x+w/2,y+230,220,170);plainText(content.periodLabel,x+w/2,y+395,periodSize,600,slide1Ink,'center');plainText(deltaValue,x+w/2,y+510,fittedSize(deltaValue,w-24,82,60,700),700,slide1Ink,'center');}",
    "function drawSlide1StoryFacts(content,dailyIcons,daylightIcon,thermometerIcon){const x=50,y=430,w=980,h=570,columnWidth=w/3,hotIcon=dailyIcons[content.hottestDay.dayIndex];box(x,y,w,h);ctx.save();ctx.strokeStyle='rgba(6,31,76,.18)';ctx.lineWidth=1.2;for(let index=1;index<3;index++){const separatorX=x+columnWidth*index;ctx.beginPath();ctx.moveTo(separatorX,y+24);ctx.lineTo(separatorX,y+h-24);ctx.stroke();}ctx.restore();drawStoryTemperatureFact(content.coldestMorning,thermometerIcon,x,y,columnWidth,175,160);drawStoryTemperatureFact(content.hottestDay,hotIcon,x+columnWidth,y,columnWidth,220,185);drawStoryDaylightFact(content.daylight,daylightIcon,x+columnWidth*2,y,columnWidth);}",
    "function drawSlide1StorySummary(content){const x=50,y=1030,w=980,h=210,visual=content.synthesis,left=x+50,maxWidth=w-100,primary=normalizeText(visual.primaryLine),secondary=normalizeText(visual.secondaryLine),primarySize=fittedSize(primary,maxWidth,36,27,700);slide1SummaryBox(x,y,w,h);plainText(primary,left,y+82,primarySize,700,slide1Ink,'left');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=5;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(left,y+107);ctx.lineTo(left+64,y+107);ctx.stroke();ctx.restore();completePlainWrap(secondary,left,y+156,maxWidth,34,27,20,560,slide1Ink,'left',2);}",
    "function drawSlide1StoryDayStrip(days,dailyIcons,content){const x=50,y=1275,w=980,h=330,columnWidth=w/days.length,coldDayIndexes=new Set(factMatches(content.coldestMorning).map(match=>match.dayIndex)),hotDayIndexes=new Set(factMatches(content.hottestDay).map(match=>match.dayIndex));slide1StripBox(x,y,w,h);ctx.save();ctx.strokeStyle='rgba(6,31,76,.14)';ctx.lineWidth=1;for(let index=1;index<days.length;index++){const separatorX=Math.round(x+columnWidth*index)+.5;ctx.beginPath();ctx.moveTo(separatorX,y+24);ctx.lineTo(separatorX,y+h-24);ctx.stroke();}ctx.restore();days.forEach(function(day,index){if(coldDayIndexes.has(day.dayIndex))drawSlide1DayMarker('COLD',x+columnWidth*index,y,columnWidth,h);if(hotDayIndexes.has(day.dayIndex))drawSlide1DayMarker('HOT',x+columnWidth*index,y,columnWidth,h);});days.forEach(function(day,index){const centerX=x+columnWidth*(index+.5),icon=dailyIcons[index];plainText(day.weekdayLabel,centerX,y+58,18,700,slide1Ink,'center');plainText(day.dayLabel,centerX,y+90,21,560,slide1Ink,'center');drawImageCentered(icon,centerX,y+185,112,86);const minLabel=String(Math.round(day.minTemperatureC))+'°',maxLabel=String(Math.round(day.maxTemperatureC))+'°';plainText(minLabel,centerX-7,y+286,25,600,slide1Ink,'right');plainText('/',centerX,y+286,18,450,slide1Ink,'center');plainText(maxLabel,centerX+7,y+286,25,600,slide1Ink,'left');});}",
    "function drawSlide1StorySignature(){plainText(model.signature||model.brand.slogan,540,1815,22,500,rgba(slide1Ink,.88),'center');ctx.save();ctx.strokeStyle=gold;ctx.lineWidth=1.5;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(516,1841);ctx.lineTo(564,1841);ctx.stroke();ctx.restore();}",
    "function drawSlide1Story(canvas,slide,background,logo,dailyIcons,daylightIcon,thermometerIcon){ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height);cover(background,canvas.width,canvas.height);overlay(canvas.width,canvas.height);drawSlide1StoryHeader(logo);drawSlide1StoryTitle(plan.overviewTitle);drawSlide1StoryFacts(plan.slide1,dailyIcons,daylightIcon,thermometerIcon);drawSlide1StorySummary(plan.slide1);drawSlide1StoryDayStrip(plan.slide1.dailyStrip,dailyIcons,plan.slide1);drawSlide1StorySignature();}",
