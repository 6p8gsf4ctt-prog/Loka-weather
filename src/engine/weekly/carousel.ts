...
import { PICTOGRAM_LIBRARY_VERSION, PICTOGRAM_STYLE, hourlyConditionToPictogram, solarPictogramDataUrl, visualIconToPictogram, weatherPictogramDataUrl } from "../../ui/pictogramLibrary";
...
'<style>:root{--ink:#12264a;--gold:#fdb515;--paper:#f3f1eb;--muted:#6f716f;--dark:#171715}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif}.wrap{max-width:1180px;margin:0 auto;padding:24px 18px 48px}.toolbar{background:#fff;border-radius:24px;padding:20px 22px;margin-bottom:22px;box-shadow:0 12px 40px rgba(18,38,74,.08)}.topline{display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{font-size:27px;font-weight:850;letter-spacing:.06em}.badge{font-size:11px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}.toolbar h1{font-size:28px;line-height:1.05;margin:20px 0 6px}.muted{font-size:13px;color:var(--muted)}.carousel{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:22px}.slide-card,.story-card{background:#fff;border-radius:24px;padding:14px;box-shadow:0 14px 46px rgba(18,38,74,.1)}.slide-head{display:flex;justify-content:space-between;gap:8px;padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.canvas-wrap{overflow:hidden;border-radius:18px;background:#d6d4cf}.canvas-wrap canvas{display:block;width:100%;height:auto}.activity-list{display:flex;flex-direction:column;gap:7px;margin:12px 2px 0}.activity{border-radius:11px;padding:8px 10px;font-size:12px;line-height:1.35}.activity.favorable{background:#edf7ef;color:#21613b}.activity.mixed{background:#faf3e3;color:#7a5b16}.activity.unfavorable{background:#f9e9e7;color:#8c302b}.secondary{width:100%;margin-top:12px;border:0;border-radius:12px;padding:12px 10px;background:#f0f0ed;color:var(--dark);font:650 12px/1 -apple-system,BlinkMacSystemFont,sans-serif;cursor:pointer}.story-card{max-width:420px;margin:28px auto 0}.story-head{padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.note{text-align:center;color:var(--muted);font-size:11px;line-height:1.5;margin:22px auto 0;max-width:760px}</style></head><body><main class="wrap"><section class="toolbar"><div class="topline"><div class="brand">LOKA!</div><div class="badge">La semaine à Tarnos · V24 · aperçu</div></div><h1>' + escapeHtml(plan.story.relay.title) + '</h1><div class="muted">' + escapeHtml(range) + ' · ' + String(plan.slides.length) + ' slide' + (plan.slides.length > 1 ? 's' : '') + '</div></section><section class="carousel" aria-label="Carrousel hebdomadaire">' + cards + '</section><section class="story-card"><div class="story-head">STORY · RELAIS DE LA PUBLICATION</div><div class="canvas-wrap"><canvas id="story-relay" width="' + String(WEEKLY_STORY_WIDTH) + '" height="' + String(WEEKLY_STORY_HEIGHT) + '" aria-label="' + escapeHtml(storyLabel) + '"></canvas></div><button class="secondary" id="download-story" type="button">Télécharger le relais Story</button></section><p class="note">Le rendu utilise le logo LOKA!, les box translucides, les pictogrammes redessinés et les fonds des 24 scènes. La Story reste un relais du carrousel ; elle ne constitue pas un bulletin autonome.</p></main><script>';
  ...
  const script = [
    ...
    "function slide1MetricBox(x,y,w,h){ctx.save();rr(x,y,w,h,28);ctx.fillStyle='rgba(255,255,255,.20)';ctx.fill();ctx.strokeStyle='rgba(255,255,255,.82)';ctx.lineWidth=1.2;ctx.stroke();ctx.restore();}",
    "function slide1StripBox(x,y,w,h){ctx.save();rr(x,y,w,h,32);ctx.fillStyle='rgba(255,255,255,.24)';ctx.fill();ctx.strokeStyle='rgba(255,255,255,.9)';ctx.lineWidth=1.4;ctx.stroke();ctx.restore();}",
    "function drawSlide1TemperatureFact(fact,icon,x,y,w,accent){slide1MetricBox(x,y,w,350);trackedText(fact.label,x+w/2,y+42,14,760,ink,1.35,'center');drawImageCentered(icon,x+w/2,y+128,112,86);text(fact.dateLabel,x+w/2,y+226,30,800,ink,'center');text(fact.temperatureLabel,x+w/2,y+315,76,820,accent,'center');}",
    "function drawSlide1DaylightFact(content,solarIcons,x,y,w){slide1MetricBox(x,y,w,350);trackedText(content.label,x+w/2,y+42,14,760,ink,1.35,'center');drawImageCentered(solarIcons.sunrise,x+w/2-52,y+120,74,56);drawImageCentered(solarIcons.sunset,x+w/2+52,y+120,74,56);text(content.deltaLabel,x+w/2,y+211,fittedSize(content.deltaLabel,w-36,39,23,820),820,gold,'center');const sunrise='Lever '+content.start.weekdayLabel+' '+content.start.sunriseLabel+' → '+content.end.weekdayLabel+' '+content.end.sunriseLabel, sunset='Coucher '+content.start.weekdayLabel+' '+content.start.sunsetLabel+' → '+content.end.weekdayLabel+' '+content.end.sunsetLabel;text(sunrise,x+w/2,y+271,fittedSize(sunrise,w-26,14,10,680),680,ink,'center');text(sunset,x+w/2,y+308,fittedSize(sunset,w-26,14,10,680),680,ink,'center');}",
    "function completeWrap(value,x,y,maxWidth,lineHeight,maxSize,minSize,weight,color,align,maxLines){const words=normalizeText(value).split(/\\s+/).filter(Boolean);for(let size=maxSize;size>=minSize;size--){ctx.save();font(size,weight);const lines=[];let line='';for(const word of words){const next=line?line+' '+word:word;if(line&&ctx.measureText(next).width>maxWidth){lines.push(line);line=word;}else line=next;}if(line)lines.push(line);ctx.restore();if(lines.length<=maxLines){const firstY=y-(lines.length-1)*lineHeight/2;ctx.save();font(size,weight);for(let index=0;index<lines.length;index++)drawFullTextLine(lines[index],x,firstY+index*lineHeight,color,align);ctx.restore();return;}}throw new Error('weekly_slide1_synthesis_does_not_fit');}",
    "function drawSlide1Facts(content,dailyIcons,solarIcons){const x=50,y=336,w=980,gap=14,cardWidth=(w-gap*2)/3,coldIcon=dailyIcons[content.coldestMorning.dayIndex],hotIcon=dailyIcons[content.hottestDay.dayIndex];drawSlide1TemperatureFact(content.coldestMorning,coldIcon,x,y,cardWidth,'#4b8fc5');drawSlide1TemperatureFact(content.hottestDay,hotIcon,x+cardWidth+gap,y,cardWidth,gold);drawSlide1DaylightFact(content.daylight,solarIcons,x+(cardWidth+gap)*2,y,cardWidth);}",
    "function drawSlide1Summary(content){box(50,710,980,145);completeWrap(content.synthesis.text,540,782,850,40,30,22,560,ink,'center',content.synthesis.maximumLines);}",
    "function drawSlide1DayMarker(kind,x,y,w,h){const accent=kind==='HOT'?gold:'#4b8fc5';ctx.save();rr(x+5,y+6,w-10,h-12,18);ctx.fillStyle=kind==='HOT'?'rgba(253,181,21,.10)':'rgba(75,143,197,.10)';ctx.fill();ctx.strokeStyle=accent;ctx.lineWidth=2.25;ctx.stroke();ctx.restore();}",
    "function drawSlide1DayStrip(days,dailyIcons,content){const x=50,y=885,w=980,h=250,columnWidth=w/days.length;days.forEach(function(day,index){if(day.dayIndex===content.coldestMorning.dayIndex)drawSlide1DayMarker('COLD',x+columnWidth*index,y,columnWidth,h);if(day.dayIndex===content.hottestDay.dayIndex)drawSlide1DayMarker('HOT',x+columnWidth*index,y,columnWidth,h);});ctx.save();ctx.strokeStyle='rgba(18,38,74,.18)';ctx.lineWidth=1.15;for(let index=1;index<days.length;index++){const separatorX=x+columnWidth*index;ctx.beginPath();ctx.moveTo(separatorX,y+18);ctx.lineTo(separatorX,y+h-18);ctx.stroke();}ctx.restore();days.forEach(function(day,index){const centerX=x+columnWidth*(index+.5),icon=dailyIcons[index];text(day.weekdayLabel,centerX,y+43,16,780,ink,'center');text(day.dayLabel,centerX,y+68,18,760,ink,'center');drawImageCentered(icon,centerX,y+137,92,70);const minLabel=String(Math.round(day.minTemperatureC))+'°',maxLabel=String(Math.round(day.maxTemperatureC))+'°';text(minLabel,centerX-7,y+216,23,780,ink,'right');text('/',centerX,y+216,17,680,ink,'center');text(maxLabel,centerX+7,y+216,23,780,gold,'left');});}",
    "function drawSlide1Overview(canvas,slide,background,logo,dailyIcons,solarIcons){ctx=canvas.getContext('2d');const width=canvas.width,height=canvas.height;ctx.clearRect(0,0,width,height);cover(background,width,height);overlay(width,height);drawHeader(logo,plan.headerDateLabel,width);box(50,160,980,150);drawOverviewTitle(plan.overviewTitle);drawSlide1Facts(plan.slide1,dailyIcons,solarIcons);drawSlide1Summary(plan.slide1);slide1StripBox(50,885,980,250);drawSlide1DayStrip(plan.slide1.dailyStrip,dailyIcons,plan.slide1);drawSignature(1274,'rgba(255,255,255,.94)');}",
    "function drawPublicationSlide(canvas,slide){if(slide.kind!=='OVERVIEW')return drawSlide(canvas,slide);const base=[load(slide.backgroundUrl,'slide1_background'),load(model.brand.logoUrl,'slide1_logo')],dailyPictograms=plan.slide1.dailyStrip.map(function(day){return load(day.pictogram.url,'slide1_pictogram_'+day.dayIndex);}),solarPictograms=[load(model.slide1SolarPictograms.sunrise,'slide1_sunrise'),load(model.slide1SolarPictograms.sunset,'slide1_sunset')];return Promise.all([...base,...dailyPictograms,...solarPictograms]).then(function(images){const dailyOffset=2,solarOffset=dailyOffset+dailyPictograms.length;drawSlide1Overview(canvas,slide,images[0],images[1],images.slice(dailyOffset,solarOffset),{sunrise:images[solarOffset],sunset:images[solarOffset+1]});});}",
