...
'<style>:root{--ink:#12264a;--gold:#fdb515;--paper:#f3f1eb;--muted:#6f716f;--dark:#171715}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif}.wrap{max-width:1180px;margin:0 auto;padding:24px 18px 48px}.toolbar{background:#fff;border-radius:24px;padding:20px 22px;margin-bottom:22px;box-shadow:0 12px 40px rgba(18,38,74,.08)}.topline{display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{font-size:27px;font-weight:850;letter-spacing:.06em}.badge{font-size:11px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}.toolbar h1{font-size:28px;line-height:1.05;margin:20px 0 6px}.muted{font-size:13px;color:var(--muted)}.carousel{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:22px}.slide-card,.story-card{background:#fff;border-radius:24px;padding:14px;box-shadow:0 14px 46px rgba(18,38,74,.1)}.slide-head{display:flex;justify-content:space-between;gap:8px;padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.canvas-wrap{overflow:hidden;border-radius:18px;background:#d6d4cf}.canvas-wrap canvas{display:block;width:100%;height:auto}.activity-list{display:flex;flex-direction:column;gap:7px;margin:12px 2px 0}.activity{border-radius:11px;padding:8px 10px;font-size:12px;line-height:1.35}.activity.favorable{background:#edf7ef;color:#21613b}.activity.mixed{background:#faf3e3;color:#7a5b16}.activity.unfavorable{background:#f9e9e7;color:#8c302b}.secondary{width:100%;margin-top:12px;border:0;border-radius:12px;padding:12px 10px;background:#f0f0ed;color:var(--dark);font:650 12px/1 -apple-system,BlinkMacSystemFont,sans-serif;cursor:pointer}.story-card{max-width:420px;margin:28px auto 0}.story-head{padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.note{text-align:center;color:var(--muted);font-size:11px;line-height:1.5;margin:22px auto 0;max-width:760px}</style></head><body><main class="wrap"><section class="toolbar"><div class="topline"><div class="brand">LOKA!</div><div class="badge">La semaine à Tarnos · V24 · aperçu</div></div><h1>' + escapeHtml(plan.story.relay.title) + '</h1><div class="muted">' + escapeHtml(range) + ' · ' + String(plan.slides.length) + ' slide' + (plan.slides.length > 1 ? 's' : '') + '</div></section><section class="carousel" aria-label="Carrousel hebdomadaire">' + cards + '</section><section class="story-card"><div class="story-head">STORY · RELAIS DE LA PUBLICATION</div><div class="canvas-wrap"><canvas id="story-relay" width="' + String(WEEKLY_STORY_WIDTH) + '" height="' + String(WEEKLY_STORY_HEIGHT) + '" aria-label="' + escapeHtml(storyLabel) + '"></canvas></div><button class="secondary" id="download-story" type="button">Télécharger le relais Story</button></section><p class="note">Le rendu utilise le logo LOKA!, les box translucides, les pictogrammes redessinés et les fonds des 24 scènes. La Story reste un relais du carrousel ; elle ne constitue pas un bulletin autonome.</p></main><script>';
  ...
  const script = [
    ...
    "function drawPreferredCard(card,mainIcon,slotIcons){const x=74,y=360,w=932,h=320;ctx.save();rr(x,y,w,h,30);ctx.fillStyle='rgba(255,255,255,.115)';ctx.fill();ctx.strokeStyle='rgba(255,255,255,.76)';ctx.lineWidth=1.2;ctx.stroke();ctx.restore();const badgeLabel='LE JOUR À PRIVILÉGIER',badgeWidth=276;ctx.save();rr(x+242,y+18,badgeWidth,40,20);ctx.fillStyle=gold;ctx.fill();ctx.restore();text(badgeLabel,x+242+badgeWidth/2,y+46,17,800,ink,'center');drawImageCentered(mainIcon,x+110,y+126,142,108);const dateLabel=card.weekdayLabel+'. '+card.dayLabel;text(dateLabel,x+205,y+134,fittedSize(dateLabel,390,66,42,820),820,ink,'left');text(card.weatherLabel,x+205,y+179,fittedSize(card.weatherLabel,390,28,18,780),780,ink,'left');ctx.save();ctx.strokeStyle='rgba(18,38,74,.23)';ctx.lineWidth=1.25;ctx.beginPath();ctx.moveTo(x+596,y+77);ctx.lineTo(x+596,y+194);ctx.moveTo(x+22,y+205);ctx.lineTo(x+w-22,y+205);ctx.stroke();ctx.restore();text(String(Math.round(card.minTemperatureC))+'°',x+676,y+137,61,820,ink,'center');text(String(Math.round(card.maxTemperatureC))+'°',x+829,y+137,61,820,gold,'center');trackedText('MIN',x+676,y+171,14,720,ink,3,'center');trackedText('MAX',x+829,y+171,14,720,ink,3,'center');const startX=x+109,columnWidth=178;card.slots.forEach(function(slot,index){const centerX=startX+columnWidth*index,icon=slotIcons[index];text(String(slot.hour).padStart(2,'0')+'h',centerX,y+235,18,720,ink,'center');drawImageCentered(icon,centerX,y+267,76,54);text(String(Math.round(slot.temperatureC))+'°',centerX+44,y+280,27,800,ink,'left');if(index<card.slots.length-1){ctx.save();ctx.strokeStyle='rgba(18,38,74,.18)';ctx.lineWidth=1.1;ctx.beginPath();ctx.moveTo(centerX+88,y+223);ctx.lineTo(centerX+88,y+300);ctx.stroke();ctx.restore();}});}",
    "function drawOverview(canvas,slide,background,logo,dailyIcons,preferredCard){ctx=canvas.getContext('2d');const width=canvas.width,height=canvas.height;ctx.clearRect(0,0,width,height);cover(background,width,height);overlay(width,height);drawHeader(logo,plan.headerDateLabel,width);box(50,160,980,150);drawOverviewTitle(plan.overviewTitle);box(50,336,980,700);if(preferredCard)drawPreferredCard(preferredCard.card,preferredCard.mainIcon,preferredCard.slotIcons);box(50,1060,980,190);drawWeeklyDayStrip(plan.dailySummaries,dailyIcons);drawSignature(1304,'rgba(255,255,255,.94)');}",
    ...
    "function drawSlide(canvas,slide){const base=[load(slide.backgroundUrl,'background_'+slide.index),load(model.brand.logoUrl,'logo')];if(slide.kind==='OVERVIEW'){const dailyPictograms=plan.dailySummaries.map(function(day){return load(day.pictogram.url,'weekly_pictogram_'+day.dayIndex);});const preferredCard=plan.dailyCardDetails.find(function(card){return card.kind==='PREFERRED';})||null;const centralPictograms=preferredCard?[load(preferredCard.pictogram.url,'central_preferred_main_'+preferredCard.dayIndex),...preferredCard.slots.map(function(slot){return load(slot.pictogram.url,'central_preferred_hour_'+slot.hour);})]:[];return Promise.all([...base,...dailyPictograms,...centralPictograms]).then(function(images){const iconOffset=2+dailyPictograms.length;const preferredVisual=preferredCard?{card:preferredCard,mainIcon:images[iconOffset],slotIcons:images.slice(iconOffset+1,iconOffset+1+preferredCard.slots.length)}:null;drawOverview(canvas,slide,images[0],images[1],images.slice(2,iconOffset),preferredVisual);});}return Promise.all([...base,load(slide.scene.pictogramUrl,'pictogram_'+slide.scene.id)]).then(function(images){drawEvent(canvas,slide,images[0],images[1],images[2]);});}",
