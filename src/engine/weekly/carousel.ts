...
'<style>:root{--ink:#12264a;--gold:#fdb515;--paper:#f3f1eb;--muted:#6f716f;--dark:#171715}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif}.wrap{max-width:1180px;margin:0 auto;padding:24px 18px 48px}.toolbar{background:#fff;border-radius:24px;padding:20px 22px;margin-bottom:22px;box-shadow:0 12px 40px rgba(18,38,74,.08)}.topline{display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{font-size:27px;font-weight:850;letter-spacing:.06em}.badge{font-size:11px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}.toolbar h1{font-size:28px;line-height:1.05;margin:20px 0 6px}.muted{font-size:13px;color:var(--muted)}.pilot{display:flex;justify-content:space-between;gap:14px;margin:16px 0 0;padding:12px 14px;border-radius:14px;font-size:12px;line-height:1.35}.pilot strong,.pilot span{display:block}.pilot span{margin-top:3px}.pilot small{align-self:center;white-space:nowrap}.pilot.pass{background:#edf7ef;color:#21613b}.pilot.review{background:#faf3e3;color:#7a5b16}.pilot.blocked{background:#f9e9e7;color:#8c302b}.carousel{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:22px}.slide-card,.story-card{background:#fff;border-radius:24px;padding:14px;box-shadow:0 14px 46px rgba(18,38,74,.1)}.slide-head{display:flex;justify-content:space-between;gap:8px;padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.canvas-wrap{overflow:hidden;border-radius:18px;background:#d6d4cf}.canvas-wrap canvas{display:block;width:100%;height:auto}.activity-list{display:flex;flex-direction:column;gap:7px;margin:12px 2px 0}.activity{border-radius:11px;padding:8px 10px;font-size:12px;line-height:1.35}.activity.favorable{background:#edf7ef;color:#21613b}.activity.mixed{background:#faf3e3;color:#7a5b16}.activity.unfavorable{background:#f9e9e7;color:#8c302b}.secondary{width:100%;margin-top:12px;border:0;border-radius:12px;padding:12px 10px;background:#f0f0ed;color:var(--dark);font:650 12px/1 -apple-system,BlinkMacSystemFont,sans-serif;cursor:pointer}.story-card{max-width:420px;margin:28px auto 0}.story-head{padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.note{text-align:center;color:var(--muted);font-size:11px;line-height:1.5;margin:22px auto 0;max-width:760px}</style></head><body><main class="wrap"><section class="toolbar"><div class="topline"><div class="brand">LOKA!</div><div class="badge">' + escapeHtml(previewBadge) + '</div></div><h1>' + escapeHtml(previewTitle) + '</h1><div class="muted">' + escapeHtml(range) + ' · ' + String(plan.slides.length) + ' slide' + (plan.slides.length > 1 ? 's' : '') + '</div>' + pilotPanel + '</section>' + carouselSection + storySection + '<p class="note">' + escapeHtml(previewNote) + '</p></main><script>';
  ...
  const script = [
    ...
    "function box(x,y,w,h){ctx.save();ctx.shadowColor='rgba(6,31,76,.13)';ctx.shadowBlur=28;ctx.shadowOffsetY=12;rr(x,y,w,h,36);const gradient=ctx.createLinearGradient(x,y,x+w,y+h);gradient.addColorStop(0,'rgba(255,255,255,.54)');gradient.addColorStop(.52,'rgba(255,255,255,.31)');gradient.addColorStop(1,'rgba(238,246,252,.20)');ctx.fillStyle=gradient;ctx.fill();ctx.shadowColor='transparent';ctx.strokeStyle='rgba(255,255,255,.92)';ctx.lineWidth=1.45;ctx.stroke();ctx.save();rr(x+2,y+2,w-4,(h-4)*.42,34);ctx.clip();const sheen=ctx.createLinearGradient(x,y,x,y+h*.48);sheen.addColorStop(0,'rgba(255,255,255,.34)');sheen.addColorStop(1,'rgba(255,255,255,.015)');ctx.fillStyle=sheen;ctx.fillRect(x+2,y+2,w-4,h*.48);ctx.restore();ctx.restore();}",
    ...
    "function overlay(width,height){const gradient=ctx.createLinearGradient(0,0,0,height);gradient.addColorStop(0,'rgba(248,252,255,.16)');gradient.addColorStop(.46,'rgba(255,255,255,.04)');gradient.addColorStop(1,'rgba(7,21,48,.44)');ctx.fillStyle=gradient;ctx.fillRect(0,0,width,height);const glow=ctx.createRadialGradient(width*.18,height*.12,20,width*.18,height*.12,width*.72);glow.addColorStop(0,'rgba(255,255,255,.30)');glow.addColorStop(1,'rgba(255,255,255,0)');ctx.fillStyle=glow;ctx.fillRect(0,0,width,height);}",
    "function capsule(label,x,y,fill,color){ctx.save();font(15,760);const w=Math.ceil(ctx.measureText(normalizeText(label)).width)+36;rr(x,y,w,34,17);ctx.fillStyle=fill;ctx.fill();ctx.restore();trackedText(label,x+w/2,y+23,15,760,color,1.3,'center');return w;}",
    "function iconHalo(cx,cy,r){ctx.save();const halo=ctx.createRadialGradient(cx-r*.25,cy-r*.35,3,cx,cy,r);halo.addColorStop(0,'rgba(255,255,255,.96)');halo.addColorStop(1,'rgba(255,255,255,.30)');ctx.fillStyle=halo;ctx.shadowColor='rgba(6,31,76,.16)';ctx.shadowBlur=20;ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.fill();ctx.restore();}",
    ...
    `function drawOverviewTitle(content){const title=normalizeText(content.title),left=${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 55},titleSize=fittedSize(title,870,48,38,760);trackedText('LA SEMAINE · L’ESSENTIEL',left,203,14,760,rgba(slide1Ink,.68),1.9,'left');plainText(title,left,263,titleSize,760,slide1Ink,'left');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=5;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 58},282);ctx.lineTo(${WEEKLY_SLIDE1_DAILY_FEED_GRID.x + 138},282);ctx.stroke();ctx.restore();}`,
    ...
    `function drawSlide1TemperatureFact(fact,icon,x,y,w,iconWidth,iconHeight){slide1MetricBox(x,y,w,${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.height});const hot=/CHAUD|MAX/i.test(fact.label),accent=hot?gold:slide1ColdBlue,labelSize=fittedSize(fact.label,w-44,18,14,760),dateSize=fittedSize(fact.dateLabel,w-44,29,18,600);ctx.save();rr(x+20,y+20,w-40,5,3);ctx.fillStyle=accent;ctx.fill();ctx.restore();trackedText(fact.label,x+w/2,y+70,labelSize,760,slide1Ink,.8,'center');iconHalo(x+w/2,y+190,86);drawImageCentered(icon,x+w/2,y+190,iconWidth,iconHeight);plainText(fact.temperatureLabel,x+w/2,y+356,fittedSize(fact.temperatureLabel,w-28,72,56,760),760,slide1Ink,'center');plainText(fact.dateLabel,x+w/2,y+414,dateSize,600,rgba(slide1Ink,.78),'center');}`,
    ...
    `function drawSlide1DaylightFact(content,daylightIcon,x,y,w){slide1MetricBox(x,y,w,${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.height});const labelSize=fittedSize(content.label,w-44,18,14,760),periodSize=fittedSize(content.periodLabel,w-44,28,18,600),deltaValue=daylightDeltaValue(content.deltaLabel);ctx.save();rr(x+20,y+20,w-40,5,3);ctx.fillStyle=slide1EditorialGold;ctx.fill();ctx.restore();trackedText(content.label,x+w/2,y+70,labelSize,760,slide1Ink,.8,'center');iconHalo(x+w/2,y+190,86);drawImageCentered(daylightIcon,x+w/2,y+190,205,154);plainText(deltaValue,x+w/2,y+356,fittedSize(deltaValue,w-28,68,52,760),760,slide1Ink,'center');plainText(content.periodLabel,x+w/2,y+414,periodSize,600,rgba(slide1Ink,.78),'center');}`,
    ...
    `function drawSlide2Number(canvas,slide,background,logo,icon){const content=slide.weeklyNumber,x=${WEEKLY_SLIDE2_DAILY_FEED_GRID.x},y=${WEEKLY_SLIDE2_DAILY_FEED_GRID.numberBox.y},w=${WEEKLY_SLIDE2_DAILY_FEED_GRID.width},h=${WEEKLY_SLIDE2_DAILY_FEED_GRID.numberBox.height};ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height);cover(background,canvas.width,canvas.height);overlay(canvas.width,canvas.height);drawSlide1Header(logo,plan.headerDateFeed,canvas.width);box(x,${WEEKLY_SLIDE2_DAILY_FEED_GRID.title.y},w,${WEEKLY_SLIDE2_DAILY_FEED_GRID.title.height});drawOverviewTitle(content);box(x,y,w,h);capsule('LE CHIFFRE DE LA SEMAINE',x+w/2-124,y+54,'rgba(6,31,76,.90)','#FFFFFF');iconHalo(x+w/2,y+275,132);drawImageCentered(icon,x+w/2,y+275,224,172);const valueSize=fittedSize(content.valueLabel,w-100,152,82,760);plainText(content.valueLabel,x+w/2,y+514,valueSize,760,slide1Ink,'center');const unitSize=fittedSize(content.unitLabel,w-120,28,18,700);trackedText(content.unitLabel,x+w/2,y+573,unitSize,700,slide1Ink,1.2,'center');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=4;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x+w/2-44,y+615);ctx.lineTo(x+w/2+44,y+615);ctx.stroke();ctx.restore();completePlainWrap(content.explanation,x+w/2,y+706,w-170,40,30,21,600,slide1Ink,'center',3);drawSlide1FeedSignature();}`,
