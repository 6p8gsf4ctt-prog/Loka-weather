...
export const WEEKLY_SLIDE1_DAILY_FEED_GRID = {
  ...
  facts: { y: 350, height: 425 },
  summary: { y: 795, height: 184 },
  dailyStrip: {
    y: 1000,
    height: 265,
    bottom: 1265
...
/**
 * Story version of the same overview master.  The header/title coordinates
 * are the daily Story coordinates; the content boxes are normalized to the
 * 9:16 canvas and keep the same internal rhythm as the publication version.
 */
export const WEEKLY_SLIDE1_STORY_GRID = {
  x: 44,
  width: 992,
  title: { x: 44, y: 200, width: 992, height: 150 },
  facts: { x: 44, y: 396, height: 520 },
  summary: { x: 44, y: 936, height: 220 },
  dailyStrip: { x: 44, y: 1176, height: 520, bottom: 1696 },
  signature: { x: 540, baseline: 1810, underlineY: 1834 }
} as const;
...
export const WEEKLY_COMPLEMENTARY_BOX_LAYOUT = {
  primary: { y: 336, height: 370 },
  secondary: { y: 728, height: 265 },
  editorial: { y: 1015, height: LOKA_DAILY_FEED_FRAME.lowerBox.bottom - 1015, bottom: LOKA_DAILY_FEED_FRAME.lowerBox.bottom }
} as const;
/**
...
function renderSlideCard(slide: WeeklyCarouselSlide, position: number, total: number): string {
  ...
  const publicationCanvas = slide.kind === "OVERVIEW"
    ? '<div class="canvas-wrap"><canvas class="carousel-canvas" data-format="publication" width="' + WEEKLY_CAROUSEL_WIDTH + '" height="' + WEEKLY_CAROUSEL_HEIGHT + '" aria-label="' + escapeHtml(label + " · publication") + '"></canvas></div>'
    : "";
  const storyCanvas = '<div class="canvas-wrap story-format"><canvas class="' + (slide.kind === "OVERVIEW" ? "weekly-story-canvas" : "carousel-canvas") + '" data-format="story" width="' + WEEKLY_STORY_WIDTH + '" height="' + WEEKLY_STORY_HEIGHT + '" aria-label="' + escapeHtml(label + " · story") + '"></canvas></div>';
  const publicationButton = slide.kind === "OVERVIEW" ? '<button class="secondary download-slide" data-format="publication" type="button">Enregistrer / partager la publication</button>' : "";
  return '<article class="slide-card" data-slide-index="' + (position - 1) + '"' + (slide.eventId ? ' data-event-id="' + escapeHtml(slide.eventId) + '"' : '') + '><div class="slide-head"><span>' +
    ...
    '</span></div>' + publicationCanvas + storyCanvas + (activities ? '<div class="activity-list">' + activities + "</div>" : "") +
    publicationButton + '<button class="secondary download-slide" data-format="story" type="button">Télécharger le format Story</button></article>';
}
...
'<style>:root{--ink:#12264a;--gold:#fdb515;--paper:#f3f1eb;--muted:#6f716f;--dark:#171715}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif}.wrap{max-width:1180px;margin:0 auto;padding:24px 18px 48px}.toolbar{background:#fff;border-radius:24px;padding:20px 22px;margin-bottom:22px;box-shadow:0 12px 40px rgba(18,38,74,.08)}.topline{display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{font-size:27px;font-weight:850;letter-spacing:.06em}.badge{font-size:11px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}.toolbar h1{font-size:28px;line-height:1.05;margin:20px 0 6px}.muted{font-size:13px;color:var(--muted)}.pilot{display:flex;justify-content:space-between;gap:14px;margin:16px 0 0;padding:12px 14px;border-radius:14px;font-size:12px;line-height:1.35}.pilot strong,.pilot span{display:block}.pilot span{margin-top:3px}.pilot small{align-self:center;white-space:nowrap}.pilot.pass{background:#edf7ef;color:#21613b}.pilot.review{background:#faf3e3;color:#7a5b16}.pilot.blocked{background:#f9e9e7;color:#8c302b}.carousel{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:22px}.slide-card,.story-card{background:#fff;border-radius:24px;padding:14px;box-shadow:0 14px 46px rgba(18,38,74,.1)}.slide-head{display:flex;justify-content:space-between;gap:8px;padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.canvas-wrap{overflow:hidden;border-radius:18px;background:#d6d4cf}.canvas-wrap canvas{display:block;width:100%;height:auto}.activity-list{display:flex;flex-direction:column;gap:7px;margin:12px 2px 0}.activity{border-radius:11px;padding:8px 10px;font-size:12px;line-height:1.35}.activity.favorable{background:#edf7ef;color:#21613b}.activity.mixed{background:#faf3e3;color:#7a5b16}.activity.unfavorable{background:#f9e9e7;color:#8c302b}.secondary{width:100%;margin-top:12px;border:0;border-radius:12px;padding:12px 10px;background:#f0f0ed;color:var(--dark);font:650 12px/1 -apple-system,BlinkMacSystemFont,sans-serif;cursor:pointer}.story-card{max-width:420px;margin:28px auto 0}.story-head{padding:3px 4px 11px;font-size:11px;font-weight:780;letter-spacing:.09em;color:var(--muted)}.note{text-align:center;color:var(--muted);font-size:11px;line-height:1.5;margin:22px auto 0;max-width:760px}</style></head><body><main class="wrap"><section class="toolbar"><div class="topline"><div class="brand">LOKA!</div><div class="badge">' + escapeHtml(previewBadge) + '</div></div><h1>' + escapeHtml(previewTitle) + '</h1><div class="muted">' + escapeHtml(range) + ' · ' + String(plan.slides.length) + ' slide' + (plan.slides.length > 1 ? 's' : '') + '</div>' + pilotPanel + '</section>' + carouselSection + storySection + '<p class="note">' + escapeHtml(previewNote) + '</p></main><script>';
  ...
  const script = [
    ...
    "function drawStoryHeader(logo,label){drawLokaLogo(logo,50,144,190,64);trackedText(String(plan.citySlug==='tarnos'?'TARNOS':plan.citySlug).toUpperCase(),540,158,25,680,slide1Ink,8,'center');text(String(label||'').toUpperCase(),1030,158,20,540,slide1Ink,'right');}",
    ...
    "function slide1MetricBox(x,y,w,h){ctx.save();rr(x,y,w,h,24);ctx.fillStyle='rgba(255,255,255,.36)';ctx.fill();ctx.strokeStyle='rgba(255,255,255,.72)';ctx.lineWidth=1;ctx.stroke();ctx.restore();}",
    "function slide1StripBox(x,y,w,h){ctx.save();rr(x,y,w,h,26);ctx.fillStyle='rgba(255,255,255,.74)';ctx.fill();ctx.strokeStyle='rgba(255,255,255,.94)';ctx.lineWidth=1.25;ctx.stroke();ctx.restore();}",
    "function slide1SummaryBox(x,y,w,h){ctx.save();rr(x,y,w,h,26);ctx.fillStyle='rgba(255,255,255,.46)';ctx.fill();ctx.strokeStyle='rgba(255,255,255,.76)';ctx.lineWidth=1;ctx.stroke();ctx.restore();}",
    ...
    `function drawSlide1TemperatureFact(fact,icon,x,y,w,iconWidth,iconHeight){slide1MetricBox(x,y,w,${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.height});const labelSize=fittedSize(fact.label,w-44,19,15,700),dateSize=fittedSize(fact.dateLabel,w-44,30,18,560);trackedText(fact.label,x+w/2,y+62,labelSize,700,slide1Ink,.35,'center');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=4;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x+w/2-28,y+82);ctx.lineTo(x+w/2+28,y+82);ctx.stroke();ctx.restore();drawImageCentered(icon,x+w/2,y+178,iconWidth,iconHeight);plainText(fact.dateLabel,x+w/2,y+315,dateSize,560,slide1Ink,'center');plainText(fact.temperatureLabel,x+w/2,y+400,fittedSize(fact.temperatureLabel,w-28,70,56,700),700,slide1Ink,'center');}`,
    ...
    `function drawSlide1DaylightFact(content,daylightIcon,x,y,w){slide1MetricBox(x,y,w,${WEEKLY_SLIDE1_DAILY_FEED_GRID.facts.height});const labelSize=fittedSize(content.label,w-44,19,15,700),periodSize=fittedSize(content.periodLabel,w-44,30,18,560),deltaValue=daylightDeltaValue(content.deltaLabel);trackedText(content.label,x+w/2,y+62,labelSize,700,slide1Ink,.35,'center');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=4;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x+w/2-28,y+82);ctx.lineTo(x+w/2+28,y+82);ctx.stroke();ctx.restore();drawImageCentered(daylightIcon,x+w/2,y+178,205,154);plainText(content.periodLabel,x+w/2,y+315,periodSize,560,slide1Ink,'center');plainText(deltaValue,x+w/2,y+400,fittedSize(deltaValue,w-28,70,56,700),700,slide1Ink,'center');}`,
    ...
    `function drawSlide1DayStrip(days,dailyIcons,content){const x=${WEEKLY_SLIDE1_DAILY_FEED_GRID.x},y=${WEEKLY_SLIDE1_DAILY_FEED_GRID.dailyStrip.y},w=${WEEKLY_SLIDE1_DAILY_FEED_GRID.width},h=${WEEKLY_SLIDE1_DAILY_FEED_GRID.dailyStrip.height},columnWidth=w/days.length,coldDayIndexes=new Set(factMatches(content.coldestMorning).map(match=>match.dayIndex)),hotDayIndexes=new Set(factMatches(content.hottestDay).map(match=>match.dayIndex));ctx.save();ctx.strokeStyle='rgba(6,31,76,.14)';ctx.lineWidth=1;for(let index=1;index<days.length;index++){const separatorX=Math.round(x+columnWidth*index)+.5;ctx.beginPath();ctx.moveTo(separatorX,y+18);ctx.lineTo(separatorX,y+h-18);ctx.stroke();}ctx.restore();days.forEach(function(day,index){if(coldDayIndexes.has(day.dayIndex))drawSlide1DayMarker('COLD',x+columnWidth*index,y,columnWidth,h);if(hotDayIndexes.has(day.dayIndex))drawSlide1DayMarker('HOT',x+columnWidth*index,y,columnWidth,h);});days.forEach(function(day,index){const centerX=x+columnWidth*(index+.5),icon=dailyIcons[index];plainText(day.weekdayLabel,centerX,y+43,17,700,slide1Ink,'center');plainText(day.dayLabel,centerX,y+70,19,560,slide1Ink,'center');drawImageCentered(icon,centerX,y+152,106,82);const minLabel=String(Math.round(day.minTemperatureC))+'°',maxLabel=String(Math.round(day.maxTemperatureC))+'°';plainText(minLabel,centerX-7,y+235,24,600,slide1Ink,'right');plainText('/',centerX,y+235,17,450,slide1Ink,'center');plainText(maxLabel,centerX+7,y+235,24,600,slide1Ink,'left');});}`,
    ...
    `function drawStoryOverviewTitle(content){const title=normalizeText(content.title),titleSize=fittedSize(title,882,58,44,760);text(title,${WEEKLY_SLIDE1_STORY_GRID.x + 55},281,titleSize,760,slide1Ink,'left');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=5;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(${WEEKLY_SLIDE1_STORY_GRID.x + 58},303);ctx.lineTo(${WEEKLY_SLIDE1_STORY_GRID.x + 138},303);ctx.stroke();ctx.restore();}`,
    `function drawStoryFact(label,icon,dateLabel,value,x,y,w,iconWidth,iconHeight){const cx=x+w/2,labelSize=fittedSize(label,w-48,22,16,700),dateSize=fittedSize(dateLabel,w-48,34,20,560),valueSize=fittedSize(value,w-34,78,56,700);trackedText(label,cx,y+70,labelSize,700,slide1Ink,.35,'center');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=4;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(cx-30,y+92);ctx.lineTo(cx+30,y+92);ctx.stroke();ctx.restore();drawImageCentered(icon,cx,y+205,iconWidth,iconHeight);plainText(dateLabel,cx,y+378,dateSize,560,slide1Ink,'center');plainText(value,cx,y+468,valueSize,700,slide1Ink,'center');}`,
    `function drawStoryFacts(content,dailyIcons,daylightIcon,thermometerIcon){const x=${WEEKLY_SLIDE1_STORY_GRID.x},y=${WEEKLY_SLIDE1_STORY_GRID.facts.y},w=${WEEKLY_SLIDE1_STORY_GRID.width},h=${WEEKLY_SLIDE1_STORY_GRID.facts.height},columnWidth=w/3,hotIcon=dailyIcons[content.hottestDay.dayIndex];ctx.save();ctx.strokeStyle='rgba(6,31,76,.19)';ctx.lineWidth=1.2;for(let index=1;index<3;index++){const separatorX=x+columnWidth*index;ctx.beginPath();ctx.moveTo(separatorX,y+20);ctx.lineTo(separatorX,y+h-20);ctx.stroke();}ctx.restore();drawStoryFact(content.coldestMorning.label,thermometerIcon,content.coldestMorning.dateLabel,content.coldestMorning.temperatureLabel,x,y,columnWidth,170,154);drawStoryFact(content.hottestDay.label,hotIcon,content.hottestDay.dateLabel,content.hottestDay.temperatureLabel,x+columnWidth,y,columnWidth,212,180);drawStoryFact(content.daylight.label,daylightIcon,content.daylight.periodLabel,daylightDeltaValue(content.daylight.deltaLabel),x+columnWidth*2,y,columnWidth,220,166);}`,
    `function drawStorySummary(content){const x=${WEEKLY_SLIDE1_STORY_GRID.x},y=${WEEKLY_SLIDE1_STORY_GRID.summary.y},w=${WEEKLY_SLIDE1_STORY_GRID.width},h=${WEEKLY_SLIDE1_STORY_GRID.summary.height},visual=content.synthesis,main=normalizeText(visual.primaryLine),secondary=normalizeText(visual.secondaryLine),left=x+62,maxWidth=w-124,primarySize=fittedSize(main,maxWidth,31,22,700),secondaryFit=fitSlide1EditorialLines(secondary,maxWidth,25,18,560,visual.secondaryMaximumLines),lineHeight=Math.round(secondaryFit.size*1.22),groupHeight=primarySize+16+4+18+secondaryFit.size,top=y+(h-groupHeight)/2,primaryBaseline=top+primarySize,accentY=primaryBaseline+16;slide1SummaryBox(x,y,w,h);text(main,left,primaryBaseline,primarySize,700,slide1Ink,'left');ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=4;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(left,accentY);ctx.lineTo(left+58,accentY);ctx.stroke();ctx.restore();secondaryFit.lines.forEach(function(line,index){text(line,left,accentY+18+secondaryFit.size+index*lineHeight,secondaryFit.size,560,slide1Ink,'left');});}`,
    `function drawStoryDayStrip(days,dailyIcons,content){const x=${WEEKLY_SLIDE1_STORY_GRID.x},y=${WEEKLY_SLIDE1_STORY_GRID.dailyStrip.y},w=${WEEKLY_SLIDE1_STORY_GRID.width},h=${WEEKLY_SLIDE1_STORY_GRID.dailyStrip.height},columnWidth=w/days.length,coldDayIndexes=new Set(factMatches(content.coldestMorning).map(match=>match.dayIndex)),hotDayIndexes=new Set(factMatches(content.hottestDay).map(match=>match.dayIndex));ctx.save();ctx.strokeStyle='rgba(6,31,76,.14)';ctx.lineWidth=1.1;for(let index=1;index<days.length;index++){const separatorX=x+columnWidth*index;ctx.beginPath();ctx.moveTo(separatorX,y+28);ctx.lineTo(separatorX,y+h-28);ctx.stroke();}ctx.restore();days.forEach(function(day,index){if(coldDayIndexes.has(day.dayIndex))drawSlide1DayMarker('COLD',x+columnWidth*index,y,columnWidth,h);if(hotDayIndexes.has(day.dayIndex))drawSlide1DayMarker('HOT',x+columnWidth*index,y,columnWidth,h);});days.forEach(function(day,index){const centerX=x+columnWidth*(index+.5),icon=dailyIcons[index];plainText(day.weekdayLabel,centerX,y+76,20,700,slide1Ink,'center');plainText(day.dayLabel,centerX,y+108,22,560,slide1Ink,'center');drawImageCentered(icon,centerX,y+268,126,96);const minLabel=String(Math.round(day.minTemperatureC))+'°',maxLabel=String(Math.round(day.maxTemperatureC))+'°';plainText(minLabel,centerX-8,y+444,28,600,slide1Ink,'right');plainText('/',centerX,y+444,19,450,slide1Ink,'center');plainText(maxLabel,centerX+8,y+444,28,600,slide1Ink,'left');});}`,
    `function drawStorySignature(){text(model.signature||model.brand.slogan,${WEEKLY_SLIDE1_STORY_GRID.signature.x},${WEEKLY_SLIDE1_STORY_GRID.signature.baseline},20,500,rgba(ink,.88),'center');ctx.save();ctx.strokeStyle=gold;ctx.lineWidth=1.4;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(514,${WEEKLY_SLIDE1_STORY_GRID.signature.underlineY});ctx.lineTo(566,${WEEKLY_SLIDE1_STORY_GRID.signature.underlineY});ctx.stroke();ctx.restore();}`,
    `function drawStoryOverview(canvas,background,logo,dailyIcons,daylightIcon,thermometerIcon){ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height);cover(background,canvas.width,canvas.height);overlay(canvas.width,canvas.height);drawStoryHeader(logo,plan.headerDateFeed);box(${WEEKLY_SLIDE1_STORY_GRID.title.x},${WEEKLY_SLIDE1_STORY_GRID.title.y},${WEEKLY_SLIDE1_STORY_GRID.title.width},${WEEKLY_SLIDE1_STORY_GRID.title.height});drawStoryOverviewTitle(plan.overviewTitle);box(${WEEKLY_SLIDE1_STORY_GRID.facts.x},${WEEKLY_SLIDE1_STORY_GRID.facts.y},${WEEKLY_SLIDE1_STORY_GRID.width},${WEEKLY_SLIDE1_STORY_GRID.facts.height});drawStoryFacts(plan.slide1,dailyIcons,daylightIcon,thermometerIcon);box(${WEEKLY_SLIDE1_STORY_GRID.summary.x},${WEEKLY_SLIDE1_STORY_GRID.summary.y},${WEEKLY_SLIDE1_STORY_GRID.width},${WEEKLY_SLIDE1_STORY_GRID.summary.height});drawStorySummary(plan.slide1);slide1StripBox(${WEEKLY_SLIDE1_STORY_GRID.dailyStrip.x},${WEEKLY_SLIDE1_STORY_GRID.dailyStrip.y},${WEEKLY_SLIDE1_STORY_GRID.width},${WEEKLY_SLIDE1_STORY_GRID.dailyStrip.height});drawStoryDayStrip(plan.slide1.dailyStrip,dailyIcons,plan.slide1);drawStorySignature();}`,
    ...
    `function drawSlide1Overview(canvas,slide,background,logo,dailyIcons,daylightIcon,thermometerIcon){drawWeeklySharedFrame(canvas,background,logo,plan.overviewTitle);drawSlide1Facts(plan.slide1,dailyIcons,daylightIcon,thermometerIcon);drawSlide1Summary(plan.slide1);slide1StripBox(${WEEKLY_SLIDE1_DAILY_FEED_GRID.x},${WEEKLY_SLIDE1_DAILY_FEED_GRID.dailyStrip.y},${WEEKLY_SLIDE1_DAILY_FEED_GRID.width},${WEEKLY_SLIDE1_DAILY_FEED_GRID.dailyStrip.height});drawSlide1DayStrip(plan.slide1.dailyStrip,dailyIcons,plan.slide1);drawSlide1FeedSignature();}`,
    ...
    `function drawComplementaryPrimaryBox(content,icon,x,y,w){const visual=content.presentation;drawImageCentered(icon,x+w/2,y+110,150,118);plainText(visual.headline,x+w/2,y+245,fittedSize(visual.headline,w-110,112,62,760),760,slide1Ink,'center');completePlainWrap(visual.subtitle,x+w/2,y+306,w-100,32,29,23,760,slide1Ink,'center',2);ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=4;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x+w/2-40,y+340);ctx.lineTo(x+w/2+40,y+340);ctx.stroke();ctx.restore();}`,
    `function drawComplementarySecondaryBox(content,x,y,w,h){const visual=content.presentation;if(visual.comparison){const leftX=x+w*.28,rightX=x+w*.72;drawComparisonItem(visual.comparison.left,leftX,y+104,y+158);drawComparisonItem(visual.comparison.right,rightX,y+104,y+158);ctx.save();ctx.strokeStyle='rgba(6,31,76,.34)';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(x+w/2,y+48);ctx.lineTo(x+w/2,y+h-48);ctx.stroke();ctx.restore();return;}trackedText('REPÈRE',x+w/2,y+64,18,720,slide1Ink,2.4,'center');completePlainWrap(content.secondaryLine,x+w/2,y+145,w-150,34,26,19,560,rgba(slide1Ink,.96),'center',2);}`,
    `function drawComplementaryEditorialBox(content,x,y,w,h){const visual=content.presentation,left=x+68;ctx.save();ctx.strokeStyle=slide1EditorialGold;ctx.lineWidth=4;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(left,y+74);ctx.lineTo(left+58,y+74);ctx.stroke();ctx.restore();completePlainWrap(visual.editorialLine,left,y+160,w-136,38,30,23,620,slide1Ink,'left',2);}`,
    `function drawComplementarySlide(canvas,slide,background,logo,icon){const content=slide.complementary,x=${WEEKLY_COMPLEMENTARY_DAILY_FEED_GRID.x},w=${WEEKLY_COMPLEMENTARY_DAILY_FEED_GRID.width},primary=${JSON.stringify(WEEKLY_COMPLEMENTARY_BOX_LAYOUT.primary)},secondary=${JSON.stringify(WEEKLY_COMPLEMENTARY_BOX_LAYOUT.secondary)},editorial=${JSON.stringify(WEEKLY_COMPLEMENTARY_BOX_LAYOUT.editorial)};if(!content||content.frame!=='WEEKLY_SHARED_V1'||!content.presentation)throw new Error('weekly_shared_frame_required');drawWeeklySharedFrame(canvas,background,logo,content);box(x,primary.y,w,primary.height);box(x,secondary.y,w,secondary.height);box(x,editorial.y,w,editorial.height);drawComplementaryPrimaryBox(content,icon,x,primary.y,w);drawComplementarySecondaryBox(content,x,secondary.y,w,secondary.height);drawComplementaryEditorialBox(content,x,editorial.y,w,editorial.height);drawSlide1FeedSignature();}`,
    `function drawStoryComplementarySlide(canvas,slide,background,logo,icon){const content=slide.complementary,x=${WEEKLY_SLIDE1_STORY_GRID.x},w=${WEEKLY_SLIDE1_STORY_GRID.width};if(!content||content.frame!=='WEEKLY_SHARED_V1'||!content.presentation)throw new Error('weekly_shared_frame_required');ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height);cover(background,canvas.width,canvas.height);overlay(canvas.width,canvas.height);drawStoryHeader(logo,plan.headerDateFeed);box(${WEEKLY_SLIDE1_STORY_GRID.title.x},${WEEKLY_SLIDE1_STORY_GRID.title.y},${WEEKLY_SLIDE1_STORY_GRID.title.width},${WEEKLY_SLIDE1_STORY_GRID.title.height});drawStoryOverviewTitle({title:content.title||slide.title});box(x,396,w,430);box(x,846,w,260);box(x,1126,w,520);drawComplementaryPrimaryBox(content,icon,x,396,w);drawComplementarySecondaryBox(content,x,846,w,260);drawComplementaryEditorialBox(content,x,1126,w,520);drawStorySignature();}`,
    `function drawStoryOutput(canvas,slide){const base=[load(slide.backgroundUrl,'story_background_'+slide.index),load(model.brand.logoUrl,'story_logo_'+slide.index)];if(slide.kind==='OVERVIEW'){const dailyPictograms=plan.slide1.dailyStrip.map(function(day){return load(day.pictogram.url,'story_weekly_pictogram_'+day.dayIndex);});const utilityPictograms=[load(model.slide1UtilityPictograms.daylight,'story_daylight'),load(model.slide1UtilityPictograms.thermometer,'story_thermometer')];return Promise.all([...base,...dailyPictograms,...utilityPictograms]).then(function(images){const dailyOffset=2,utilityOffset=dailyOffset+dailyPictograms.length;drawStoryOverview(canvas,images[0],images[1],images.slice(dailyOffset,utilityOffset),images[utilityOffset],images[utilityOffset+1]);});}if(slide.kind==='COMPLEMENTARY_NUMBER'||slide.kind==='COMPLEMENTARY_PRACTICAL'||slide.kind==='COMPLEMENTARY_DETAIL'){return Promise.all([...base,load(slide.complementaryPictogramUrl,'story_complementary_pictogram_'+slide.index)]).then(function(images){drawStoryComplementarySlide(canvas,slide,images[0],images[1],images[2]);});}return Promise.all([...base,load(model.weeklyNumberPictogram,'story_weekly_number_pictogram')]).then(function(images){drawStoryComplementarySlide(canvas,slide,images[0],images[1],images[2]);});}`,
    ...
    "Promise.all(plan.slides.map(function(slide,index){const canvas=slide.kind==='OVERVIEW'?document.querySelector('[data-slide-index=\\\"'+index+'\\\"] canvas[data-format=publication]'):null;return canvas?drawPublicationSlide(canvas,slide):Promise.resolve();})).catch(function(error){console.error(error);});",
    "Promise.all(plan.slides.map(function(slide,index){const canvas=storyCanvases[index];return canvas?drawStoryOutput(canvas,slide):Promise.resolve();})).catch(function(error){console.error(error);});",
    "document.querySelectorAll('.download-slide').forEach(function(button){button.addEventListener('click',function(){const card=button.closest('.slide-card'),index=Number(card.dataset.slideIndex),format=button.dataset.format,canvas=card.querySelector('canvas[data-format='+format+']');if(canvas)shareCanvas(canvas,'loka-semaine-'+String(index+1).padStart(2,'0')+'-'+format+'.png');});});"
  ].join("\n");
