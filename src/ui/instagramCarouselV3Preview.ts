...
export function renderInstagramCarouselV3Preview(payload: OfficialPublicPayloadV24, city: CityConfig, options: { embedded?: boolean; studioPrimary?: boolean; studioOfficial?: boolean; automationRender?: boolean; automationPage?: 1 | 2 } = {}): string {
  ...
  const studioPrimary = options.studioPrimary === true;
...
:root{--ink:#17212b;--muted:#73716c;--paper:#ecebe7}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif;padding:${embedded ? "0" : "max(14px,env(safe-area-inset-top)) 12px max(26px,env(safe-area-inset-bottom))"}}.wrap{width:min(100%,580px);margin:auto}.toolbar{background:#fff;border-radius:24px;padding:18px;margin-bottom:16px}.badge{display:inline-block;background:#fff1cc;color:#76530a;font-size:10px;font-weight:800;letter-spacing:.08em;padding:6px 9px;border-radius:999px}.toolbar h1{font-size:23px;margin:12px 0 6px}.muted{font-size:12px;color:var(--muted);line-height:1.5}.actions,.export-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px}a,button{border:0;border-radius:14px;padding:13px 10px;text-decoration:none;text-align:center;font:650 13px/1 -apple-system,BlinkMacSystemFont,sans-serif;cursor:pointer}.secondary{background:#f1f1ee;color:#171715}.primary{background:#171715;color:#fff}.visual{margin-bottom:18px}.head{display:flex;justify-content:space-between;align-items:end;padding:0 4px 8px}.title{font-size:13px;font-weight:800;letter-spacing:.08em}.size{font-size:11px;color:var(--muted)}.canvas-wrap{border-radius:26px;overflow:hidden;background:#d8d8d4;box-shadow:0 18px 70px rgba(0,0,0,.14)}canvas{display:block;width:100%;height:auto}.note{font-size:11px;line-height:1.45;color:#85817b;padding:9px 6px 0}.embedded-badge{background:#fff;border-radius:18px;padding:12px 14px;margin-bottom:14px;font-size:11px;line-height:1.45;color:var(--muted)}@media(max-width:420px){.actions,.export-actions{grid-template-columns:1fr}}${automationRender ? `.toolbar,.embedded-badge,.head,.export-actions,.note{display:none!important}body{padding:0!important;background:#000!important}.wrap{width:1080px!important;max-width:none!important;margin:0!important}.visual{display:none!important;margin:0!important}.visual[data-page="${automationPage}"]{display:block!important}.canvas-wrap{width:1080px!important;height:1350px!important;border-radius:0!important;box-shadow:none!important;overflow:hidden!important}canvas{width:1080px!important;height:1350px!important}` : ""}</style></head><body><div class="wrap">${embedded ? (studioOfficial ? `<div class="embedded-badge"><strong>V3 · FORMAT OFFICIEL MANUEL</strong><br>Exports officiels manuels actifs. L’automatisation reste encore sur le V2 et le rollback V2 reste disponible.</div>` : studioPrimary ? `<div class="embedded-badge"><strong>V3 · FORMAT PRINCIPAL DU STUDIO</strong><br>Exports manuels actifs. La publication automatique reste encore sur le V2.</div>` : `<div class="embedded-badge"><strong>V3 PARALLÈLE · NON OFFICIEL</strong><br>Comparaison uniquement. La publication V2 reste la référence de production.</div>`) : `<div class="toolbar"><span class="badge">${studioOfficial ? "V3 OFFICIEL · MANUEL" : studioPrimary ? "V3 STUDIO · PRINCIPAL MANUEL" : "V3 PREVIEW · NON PUBLIÉ"}</span><h1>Carrousel LOKA — Page 1 + Page 2</h1><div class="muted">${studioOfficial ? "Format officiel de publication manuelle du Studio, alimenté par <code>analysis V3</code>. L’automatisation reste sur V2 jusqu’à sa certification dédiée et le rollback V2 reste disponible." : studioPrimary ? "Format principal de travail du Studio, alimenté par <code>analysis V3</code>. Les exports sont manuels et l’automatisation V2 reste inchangée." : "Aperçu isolé alimenté par <code>analysis V3</code>. Le Studio, les Stories et la publication V2 actuels ne sont pas modifiés."}</div><div class="actions"><a class="secondary" href="/instagram">${studioOfficial || studioPrimary ? "Retour au Studio" : "Studio V2 actuel"}</a><a class="secondary" href="/admin">Admin</a></div></div>`}
<div class="visual" data-page="1"><div class="head"><div class="title">PAGE 1 · MÉTÉO UTILE</div><div class="size">1080 × 1350 · 4:5</div></div><div class="canvas-wrap"><canvas id="page1" width="1080" height="1350"></canvas></div><div class="export-actions"><button class="primary" id="exportPage1">Exporter Page 1</button><button class="secondary" id="openPreview">Ouvrir le preview V3</button></div></div>
<div class="visual" data-page="2"><div class="head"><div class="title">PAGE 2 · COMPRENDRE LA JOURNÉE</div><div class="size">1080 × 1350 · 4:5</div></div><div class="canvas-wrap"><canvas id="page2" width="1080" height="1350"></canvas></div><div class="export-actions"><button class="primary" id="exportPage2">Exporter Page 2</button><button class="secondary" id="exportBoth">Exporter les 2 pages</button></div><div class="note">${studioOfficial ? "Ces deux pages constituent le format officiel de publication manuelle. L’automatisation V3 reste désactivée pour cette phase." : "Ces exports V3 sont manuels et n’alimentent aucune publication automatique."}</div></div></div><script>
