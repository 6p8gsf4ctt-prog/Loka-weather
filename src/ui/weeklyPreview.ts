...
function renderWeeklyCandidatePreviewLegacy(params: URLSearchParams): string {
...
/** Mirrors the final three-box editorial composition before publication. */
function renderWeeklyCandidatePreviewFourBoxesLegacy(params: URLSearchParams): string {
  const read = (key: string, fallback: string): string => params.get(key)?.trim() || fallback;
  const title = read("title", "Information météo");
  const value = read("value", "—");
  const primary = read("primary", "Information météo remarquable");
  const secondary = read("secondary", "Information détectée par le moteur éditorial.");
  const source = read("source", "Source moteur LOKA.");
  const date = read("date", "Semaine à venir");
  const status = read("status", "Aperçu éditorial");
  const leftValue = read("leftValue", ""); const leftLabel = read("leftLabel", "");
  ...
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Aperçu · LOKA</title><style>:root{--ink:#102b55;--gold:#c39a3d;--paper:#f2f0ea}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif;display:grid;place-items:center;padding:24px}.page{width:min(100%,620px)}.back{display:inline-block;margin:0 0 14px;color:var(--ink);font-size:14px;font-weight:750;text-decoration:none}.slide{aspect-ratio:3/4;background:linear-gradient(145deg,#f5f0df 0%,#dce6ee 42%,#17365e 100%);border-radius:28px;padding:30px;box-shadow:0 18px 50px rgba(16,43,85,.18);position:relative;overflow:hidden}.slide:before{content:"";position:absolute;inset:-25%;background:radial-gradient(ellipse at 25% 20%,rgba(255,255,255,.85),transparent 43%),radial-gradient(ellipse at 70% 48%,rgba(255,255,255,.5),transparent 38%);transform:rotate(-10deg)}.content{position:relative;z-index:1;height:100%;display:flex;flex-direction:column}.header{display:flex;justify-content:space-between;align-items:center;font-size:13px;font-weight:800;letter-spacing:.18em}.logo{font-size:25px}.logo span{color:#f2b317}.date{font-size:11px;letter-spacing:.08em}.title,.primary-box,.secondary-box,.editorial-box{border:1px solid rgba(255,255,255,.82);border-radius:18px;background:rgba(255,255,255,.16)}.title{margin-top:30px;padding:17px 20px;font-size:20px;font-weight:850}.title:after,.accent{content:"";display:block;width:58px;height:3px;background:var(--gold);margin-top:12px}.primary-box{margin-top:16px;height:33%;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:20px}.icon{font-size:42px;line-height:1}.value{margin-top:12px;font-size:clamp(48px,11vw,76px);font-weight:850;line-height:1}.primary{margin-top:8px;font-size:clamp(16px,3.4vw,23px);font-weight:850;line-height:1.12}.accent{margin:14px auto 0}.secondary-box{margin-top:14px;height:22%;padding:18px 24px}.comparison{display:grid;grid-template-columns:1fr 1px 1fr;align-items:center;text-align:center}.comparison i{width:1px;height:66px;background:rgba(16,43,85,.3)}.comparison div{display:grid;gap:5px}.comparison b{font-size:clamp(23px,5.8vw,37px)}.comparison span{font-size:11px;font-weight:750}.reference{display:grid;place-items:center;text-align:center}.reference small{font-weight:850;letter-spacing:.16em}.reference p{margin:7px 0 0;font-size:13px;line-height:1.3}.editorial-box{margin-top:14px;flex:1;padding:24px 25px}.editorial-box .accent{margin:0}.editorial{margin:17px 0 0;font-size:clamp(16px,3.5vw,21px);line-height:1.3;font-weight:650}.source{margin-top:18px;font-size:10px;line-height:1.35;color:#4f6077}.status{margin-top:7px;color:#7a5b16;font-weight:750;text-transform:uppercase}</style></head><body><main class="page"><a class="back" href="/weekly-preview?start=${encodeURIComponent(date)}">← Retour au classement</a><article class="slide"><div class="content"><div class="header"><div class="logo">LOKA<span>!</span></div><div class="date">${escapeHtml(date)}</div></div><div class="title">${escapeHtml(title)}</div><div class="primary-box"><div class="icon">◉</div><div class="value">${escapeHtml(value)}</div><div class="primary">${escapeHtml(primary)}</div><div class="accent"></div></div>${secondaryBox}<div class="editorial-box"><div class="accent"></div><p class="editorial">${escapeHtml(secondary)}</p><div class="source">${escapeHtml(source)}<div class="status">${escapeHtml(status)} · aperçu sans publication</div></div></div></div></article></main></body></html>`;
}

/** Mirrors the locked title / information / editorial three-box composition. */
export function renderWeeklyCandidatePreview(params: URLSearchParams): string {
  ...
  const leftValue = read("leftValue", ""); const leftLabel = read("leftLabel", "");
  const rightValue = read("rightValue", ""); const rightLabel = read("rightLabel", "");
  const supporting = leftValue && rightValue
    ? `<div class="comparison"><div><b>${escapeHtml(leftValue)}</b><span>${escapeHtml(leftLabel)}</span></div><i></i><div><b>${escapeHtml(rightValue)}</b><span>${escapeHtml(rightLabel)}</span></div></div>`
    : `<div class="reference"><small>REPÈRE</small><p>${escapeHtml(source)}</p></div>`;
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Aperçu · LOKA</title><style>:root{--ink:#102b55;--gold:#c39a3d;--paper:#f2f0ea}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:var(--paper);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif;display:grid;place-items:center;padding:24px}.page{width:min(100%,620px)}.back{display:inline-block;margin:0 0 14px;color:var(--ink);font-size:14px;font-weight:750;text-decoration:none}.slide{aspect-ratio:3/4;background:linear-gradient(145deg,#f5f0df 0%,#dce6ee 42%,#17365e 100%);border-radius:28px;padding:30px;box-shadow:0 18px 50px rgba(16,43,85,.18);position:relative;overflow:hidden}.slide:before{content:"";position:absolute;inset:-25%;background:radial-gradient(ellipse at 25% 20%,rgba(255,255,255,.85),transparent 43%),radial-gradient(ellipse at 70% 48%,rgba(255,255,255,.5),transparent 38%);transform:rotate(-10deg)}.content{position:relative;z-index:1;height:100%;display:flex;flex-direction:column}.header{display:flex;justify-content:space-between;align-items:center;font-size:13px;font-weight:800;letter-spacing:.18em}.logo{font-size:25px}.logo span{color:#f2b317}.date{font-size:11px;letter-spacing:.08em}.title,.content-box,.editorial-box{border-style:solid;border-width:1px}.title{margin-top:30px;padding:17px 20px;font-size:20px;font-weight:850;border-color:rgba(255,255,255,.82);border-radius:18px;background:rgba(255,255,255,.16)}.title:after,.accent{content:"";display:block;width:32px;height:2px;background:var(--gold);margin-top:12px}.title:after{width:58px;height:3px}.content-box{margin-top:16px;height:54%;display:flex;flex-direction:column;align-items:center;text-align:center;padding:23px 22px;border-color:rgba(255,255,255,.72);border-radius:14px;background:rgba(255,255,255,.36)}.icon{font-size:42px;line-height:1}.value{margin-top:12px;font-size:clamp(46px,10.5vw,72px);font-weight:720;line-height:1}.primary{margin-top:7px;font-size:clamp(15px,3.1vw,21px);font-weight:700;line-height:1.12}.accent{margin:12px auto 0}.comparison{width:100%;display:grid;grid-template-columns:1fr 1px 1fr;align-items:center;text-align:center;margin-top:32px;padding:0 8px}.comparison i{width:1px;height:42px;background:rgba(16,43,85,.23)}.comparison div{display:grid;gap:5px}.comparison b{font-size:clamp(19px,4.8vw,31px);font-weight:700}.comparison span{font-size:10px;font-weight:650}.reference{margin:32px 0 0;display:grid;place-items:center;text-align:center}.reference small{font-weight:700;letter-spacing:.16em}.reference p{margin:7px 0 0;font-size:12px;line-height:1.3}.editorial-box{margin-top:16px;height:14%;flex:none;padding:14px 25px;border-color:rgba(255,255,255,.76);border-radius:15px;background:rgba(255,255,255,.46);display:flex;flex-direction:column;justify-content:center}.editorial-box .accent{margin:0}.editorial{margin:10px 0 0;font-size:clamp(15px,3.2vw,20px);line-height:1.28;font-weight:650}.source{display:none}.status{margin-top:7px;color:#7a5b16;font-weight:750;text-transform:uppercase}</style></head><body><main class="page"><a class="back" href="/weekly-preview?start=${encodeURIComponent(date)}">← Retour au classement</a><article class="slide"><div class="content"><div class="header"><div class="logo">LOKA<span>!</span></div><div class="date">${escapeHtml(date)}</div></div><div class="title">${escapeHtml(title)}</div><div class="content-box"><div class="icon">◉</div><div class="value">${escapeHtml(value)}</div><div class="primary">${escapeHtml(primary)}</div><div class="accent"></div>${supporting}</div><div class="editorial-box"><div class="accent"></div><p class="editorial">${escapeHtml(secondary)}</p><div class="source">${escapeHtml(source)}<div class="status">${escapeHtml(status)} · aperçu sans publication</div></div></div></div></article></main></body></html>`;
}
