...
</style></head><body><div class="wrap"><div class="toolbar"><div class="topline"><div class="brand">LOKA!</div><div class="badge">Météo officielle</div></div><h1>Visuel Instagram</h1><div class="muted" id="summary">Tarnos · visuel du jour</div><div class="actions"><button class="primary" id="share">Partager / enregistrer</button><a class="secondary" href="/admin">Retour</a></div></div><div class="canvas-wrap"><canvas id="post" width="1080" height="1350"></canvas></div><div class="note">Le visuel utilise uniquement la météo officielle actuellement publiée par LOKA.</div></div><script>
const state=${data};const f=state.forecast;const cfg={timezone:state.timezone};const visual=state.visual;const solar=state.solar||{dawn:null,sunrise:null,solarNoon:null,sunset:null,dusk:null,daylightMinutes:null,daylightDeltaMinutes:null};const canvas=document.getElementById('post');const ctx=canvas.getContext('2d');
...
async function draw(){
  ctx.clearRect(0,0,1080,1350);const bg=await load(visual.masterUrl);ctx.drawImage(bg,0,0,1080,1350);
  const dark=visual.dark,ink=dark?'#ffffff':'#0a1830',accent=dark?'#d9e4ff':'#6d86bc';
  ...
  text('LOKA!',48,78,50,360,ink);text(String(f.city||'Tarnos').toUpperCase(),540,72,23,680,ink,'center');text(dateLabel(f.date),1032,72,18,600,ink,'right');
  drawWeatherIcon(sceneIconKind(visual.title),342,193,.78,accent);text(visual.title,420,218,66,780,ink);wrap(f.mainVerdict||f.subtitle||'',540,282,760,34,27,470,ink,'center',2);
  ...
  const hourly=Array.isArray(f.hourly)?f.hourly.slice(0,9):[],row1=hourly.slice(0,5),row2=hourly.slice(5,9);
  drawHourlyRow(row1,[150,338,540,728,916].slice(0,row1.length),394,445,487,548,ink,accent);
  drawHourlyRow(row2,[208,418,632,842].slice(0,row2.length),620,668,710,765,ink,accent);
  drawSolarTimeline(ink);
  glassPanel(104,1046,872,222,30,dark);
  const lines=Array.isArray(f.summaryLines)?f.summaryLines.filter(x=>typeof x==='string'&&x.trim()).slice(0,2):[];
  const first=lines[0]||('Températures comprises entre '+String(f.tempMinC)+' et '+String(f.tempMaxC)+'°.');const second=lines[1]||(f.rainVerdict||'');
  ...
  const sunText=solar.sunrise&&solar.sunset?' · ☀ '+solar.sunrise+' / '+(solar.solarNoon||'—')+' / '+solar.sunset:'';document.getElementById('summary').textContent=String(f.city||'Tarnos')+' · '+visual.title+' · '+String(f.tempMinC)+'° / '+String(f.tempMaxC)+'°'+sunText;
}
function pngFile(){const b=atob(canvas.toDataURL('image/png').split(',')[1]),u=new Uint8Array(b.length);for(let i=0;i<b.length;i++)u[i]=b.charCodeAt(i);return new File([u],'loka-'+String(f.date||'meteo')+'.png',{type:'image/png'})}
