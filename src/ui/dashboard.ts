...
</style></head><body><div class="box"><strong>LOKA!</strong><h1>Lancer Tarnos maintenant</h1><p>Le token reste uniquement dans cette page et n’est pas enregistré.</p><input id="token" type="password" placeholder="ADMIN_TOKEN"><button id="run">Générer la météo</button><button class="secondary" id="shadow">Comparer Legacy / V24</button><button class="secondary" id="metrics">Analyser Shadow V24</button><button class="secondary" id="readinessBtn">Évaluer readiness V24</button><a class="ig" href="/instagram">Créer le visuel Instagram</a><a class="ig" href="/instagram24">Contrôler le Studio V24</a><pre id="out">Prêt.</pre>
...
document.getElementById('run').onclick=async()=>{
  ...
  try{
    ...
    if(r.ok){await loadShadow();await loadMetrics();await loadReadiness()}
