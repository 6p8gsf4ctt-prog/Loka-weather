...
if(warning){warning.textContent='Étape 4 : les retouches peuvent maintenant être enregistrées dans la mémoire éditoriale LOKA!. « Rétablir l’officiel » remet les champs d’origine ; enregistre ensuite pour supprimer une ancienne retouche mémorisée.';warning.insertAdjacentElement('beforebegin',panel);}else{studio.appendChild(panel);}
...
function currentDrafts(){const paragraph1=field('feedParagraph1');const paragraph2=field('feedParagraph2');return{story:{subtitle:field('storySubtitle'),primaryLine:field('storyPrimary'),secondaryLine:field('storySecondary'),hashtags:field('storyHashtags')},feed:{subtitle:field('feedSubtitle'),primaryLine:field('feedPrimary'),secondaryLine:field('feedSecondary'),paragraph1,paragraph2,caption:buildCaption(paragraph1,paragraph2),hashtags:field('feedHashtags')}};}
function applyFeedback(feedback){if(!feedback)return false;const story=feedback.storyEdited;const feed=feedback.feedEdited;if(story){document.getElementById('storySubtitle').value=story.subtitle||'';document.getElementById('storyPrimary').value=story.primaryLine||'';document.getElementById('storySecondary').value=story.secondaryLine||'';document.getElementById('storyHashtags').value=story.hashtags||'';}if(feed){document.getElementById('feedSubtitle').value=feed.subtitle||'';document.getElementById('feedPrimary').value=feed.primaryLine||'';document.getElementById('feedSecondary').value=feed.secondaryLine||'';document.getElementById('feedParagraph1').value=feed.paragraph1||'';document.getElementById('feedParagraph2').value=feed.paragraph2||'';document.getElementById('feedHashtags').value=feed.hashtags||'';}document.getElementById('applyEditorial')?.click();window.__LOKA_EDITORIAL_DRAFT=window.__LOKA_EDITORIAL_DRAFT||{};window.__LOKA_EDITORIAL_DRAFT.persisted=true;return !!(story||feed);}
...
refreshAuth();
window.__LOKA_EDITORIAL_PERSISTENCE={version:'1.0',api:API,storageKey:STORAGE_KEY};
