...
function currentDrafts(){const paragraph1=field('feedParagraph1');const paragraph2=field('feedParagraph2');const engagementFormat=document.getElementById('engagementFormat')?.value==='QUESTION'?'QUESTION':'POLL';return{story:{subtitle:legacySubtitle('story'),primaryLine:field('storyPrimary'),secondaryLine:field('storySecondary'),hashtags:field('storyHashtags'),engagementFormat,engagementQuestion:field('engagementQuestion'),engagementOptionA:field('engagementOptionA'),engagementOptionB:field('engagementOptionB')},feed:{subtitle:legacySubtitle('feed'),primaryLine:field('feedPrimary'),secondaryLine:field('feedSecondary'),paragraph1,paragraph2,caption:buildCaption(paragraph1,paragraph2),hashtags:field('feedHashtags')}};}
function applyFeedback(feedback){if(!feedback)return false;const story=feedback.storyEdited;const feed=feedback.feedEdited;if(story){setLegacySubtitle('story',story.subtitle??m.visual?.subtitle??'');document.getElementById('storyPrimary').value=story.primaryLine||'';document.getElementById('storySecondary').value=story.secondaryLine||'';document.getElementById('storyHashtags').value=story.hashtags||'';const format=document.getElementById('engagementFormat');if(format)format.value=story.engagementFormat||m.engagement?.format||'POLL';const question=document.getElementById('engagementQuestion');if(question)question.value=story.engagementQuestion??m.engagement?.question??'';const optionA=document.getElementById('engagementOptionA');if(optionA)optionA.value=story.engagementOptionA??m.engagement?.options?.[0]??'';const optionB=document.getElementById('engagementOptionB');if(optionB)optionB.value=story.engagementOptionB??m.engagement?.options?.[1]??'';format?.dispatchEvent(new Event('change'));}if(feed){setLegacySubtitle('feed',feed.subtitle??m.visual?.subtitle??'');document.getElementById('feedPrimary').value=feed.primaryLine||'';document.getElementById('feedSecondary').value=feed.secondaryLine||'';document.getElementById('feedParagraph1').value=feed.paragraph1||'';document.getElementById('feedParagraph2').value=feed.paragraph2||'';document.getElementById('feedHashtags').value=feed.hashtags||'';}document.getElementById('applyEditorial')?.click();window.__LOKA_EDITORIAL_DRAFT=window.__LOKA_EDITORIAL_DRAFT||{};window.__LOKA_EDITORIAL_DRAFT.persisted=true;return !!(story||feed);}
...
refreshAuth();
window.__LOKA_EDITORIAL_PERSISTENCE={version:'1.2',api:API,storageKey:STORAGE_KEY,legacySubtitle:'PRESERVED_HIDDEN'};
