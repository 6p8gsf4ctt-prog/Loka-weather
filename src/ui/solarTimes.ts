...
export interface SolarPresentation extends SolarTimes {
  sunriseDeltaMinutes: number | null;
  sunsetDeltaMinutes: number | null;
}
...
function solarUtcHours(
  ...
  const t = n + (rising ? (6 - lngHour) / 24 : (18 - lngHour) / 24);
...
function midpointUtcHours(start: number | null, end: number | null): number | null {
...
function daylightMinutes(sunriseUtc: number | null, sunsetUtc: number | null): number | null {
  if (sunriseUtc === null || sunsetUtc === null) return null;
  let adjustedSunset = sunsetUtc;
  if (adjustedSunset < sunriseUtc) adjustedSunset += 24;
  return Math.round((adjustedSunset - sunriseUtc) * 60);
}

function previousDateParts(year: number, month: number, day: number): { year: number; month: number; day: number } {
...
function formatUtcHours(
  ...
  const milliseconds =
    Date.UTC(year, month - 1, day, 0, 0, 0) + Math.round(utcHours * 60 * 60 * 1000);
  try {
...
export function calculateSolarTimes(
  ...
  const dawnUtc = solarUtcHours(year, month, day, latitude, longitude, true, CIVIL_TWILIGHT_ZENITH);
  const sunriseUtc = solarUtcHours(year, month, day, latitude, longitude, true, OFFICIAL_ZENITH);
  const sunsetUtc = solarUtcHours(year, month, day, latitude, longitude, false, OFFICIAL_ZENITH);
  const duskUtc = solarUtcHours(year, month, day, latitude, longitude, false, CIVIL_TWILIGHT_ZENITH);
  const solarNoonUtc = midpointUtcHours(sunriseUtc, sunsetUtc);
  const todayDaylightMinutes = daylightMinutes(sunriseUtc, sunsetUtc);

  const previous = previousDateParts(year, month, day);
  const previousSunriseUtc = solarUtcHours(
    previous.year,
    previous.month,
    previous.day,
    latitude,
  ...
  const previousSunsetUtc = solarUtcHours(
    previous.year,
    previous.month,
    previous.day,
    latitude,
  ...
  const previousDaylightMinutes = daylightMinutes(previousSunriseUtc, previousSunsetUtc);
  const daylightDeltaMinutes =
    todayDaylightMinutes !== null && previousDaylightMinutes !== null
      ? todayDaylightMinutes - previousDaylightMinutes
      : null;
  ...
  return {
    ...
    daylightMinutes: todayDaylightMinutes,
...
export function solarPresentation(city: CityConfig, date: string): SolarPresentation {
  const current = calculateSolarTimes(date, city.latitude, city.longitude, city.timezone);
  const previous = calculateSolarTimes(addDays(date, -1), city.latitude, city.longitude, city.timezone);
  return {
    ...current,
    sunriseDeltaMinutes: clockDeltaMinutes(current.sunrise, previous.sunrise),
    sunsetDeltaMinutes: clockDeltaMinutes(current.sunset, previous.sunset)
  };
}
