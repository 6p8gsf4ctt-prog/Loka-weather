...
function centeredTitleLayout(value,maxWidth,maxSize,minSize,weight){const label=normalizeText(value);for(let size=maxSize;size>=minSize;size--){const width=measureWidth(label,size,weight);if(width<=maxWidth)return{lines:[label],size,lineHeight:Math.round(size*.98),width};}const fit=fitLines(label,maxWidth,2,Math.min(maxSize,48),minSize,weight);const widths=fit.lines.map(line=>measureWidth(line,fit.size,weight));return{lines:fit.lines,size:fit.size,lineHeight:Math.round(fit.size*.98),width:Math.max(...widths)};}
...
function drawStoryGeneral(mainIcon){const x=44,y=224,w=992,h=190;box(x,y,w,h);drawCenteredGeneralGroup(mainIcon,x,y,w,h,146,118,30,44,52,34,48);}
function drawFeedGeneral(mainIcon){const x=50,y=160,w=980,h=185;box(x,y,w,h);drawCenteredGeneralGroup(mainIcon,x,y,w,h,136,110,28,40,48,32,44);}
function drawStoryHours(slots,icons){const x=44,y=460,w=992,h=704;box(x,y,w,h);const colW=w/5;const rowTop=[488,834],rowBottom=[812,1138];separator(x+20,826,x+w-20,826);for(let row=0;row<2;row++){for(let i=1;i<5;i++){const sx=x+colW*i;separator(sx,rowTop[row]+18,sx,rowBottom[row]-18);}for(let c=0;c<5;c++){const idx=row*5+c,item=slots[idx];const cx=x+colW*(c+.5),base=rowTop[row];text(String(item.hour).padStart(2,'0')+'h',cx,base+52,26,600,INK,'center');if(!item.missing&&icons[idx])drawImageCentered(icons[idx],cx,base+141,126,102);text(String(item.temperatureC)+(item.missing?'':'°'),cx,base+242,43,700,INK,'center');}}}
function drawFeedHours(slots,icons){const x=50,y=371,w=980,h=500;box(x,y,w,h);const colW=w/5;const rowTop=[391,635],rowBottom=[611,855];separator(x+20,623,x+w-20,623);for(let row=0;row<2;row++){for(let i=1;i<5;i++){const sx=x+colW*i;separator(sx,rowTop[row]+10,sx,rowBottom[row]-10);}for(let c=0;c<5;c++){const idx=row*5+c,item=slots[idx];const cx=x+colW*(c+.5),base=rowTop[row];text(String(item.hour).padStart(2,'0')+'h',cx,base+32,22,600,INK,'center');if(!item.missing&&icons[idx])drawImageCentered(icons[idx],cx,base+95,100,82);text(String(item.temperatureC)+(item.missing?'':'°'),cx,base+186,36,700,INK,'center');}}}
...
function drawStoryComments(){const visual=m.storyVisual||m.visual;const x=44,y=1203,w=992,h=232;box(x,y,w,h);drawEditorialSummary(visual,x,y,w,h,34,23,25,19,58);}
function drawFeedComments(){const visual=m.feedVisual||m.visual;const x=50,y=900,w=980,h=175;box(x,y,w,h);drawEditorialSummary(visual,x,y,w,h,29,20,21,17,52);}
