...
</style></head><body><div class="wrap"><div class="toolbar"><div class="topline"><div class="brand">LOKA!</div><div class="badge">Pictogrammes Premium · V24</div></div><h1>Studio Instagram</h1><div class="muted" id="summary">${payload.city} · scène ${String(payload.scene.id).padStart(2,"0")}</div></div>
...
const storyCanvas=document.getElementById('story');const feedCanvas=document.getElementById('feed');const storyCtx=storyCanvas.getContext('2d');const feedCtx=feedCanvas.getContext('2d');let ctx=storyCtx;const INK=m.pictogramLibrary.ink;const GOLD=m.pictogramLibrary.gold;const CANVAS_FONT='"Helvetica Neue",Arial,sans-serif';
...
function drawStoryGeneral(mainIcon){const x=44,y=224,w=992,h=250;box(x,y,w,h);drawImageCentered(mainIcon,171,322,170,138);drawTitleBlock(m.scene.title,290,330,294,480,66,31,800,INK);drawSubtitleBlock(m.visual.subtitle,290,418,407,480,27,20,rgba(INK,.99));text(String(m.temps.minC)+'° — '+String(m.temps.maxC)+'°',976,354,49,600,INK,'right');}
function drawFeedGeneral(mainIcon){const x=50,y=160,w=980,h=240;box(x,y,w,h);drawImageCentered(mainIcon,170,270,158,128);drawTitleBlock(m.scene.title,290,286,252,480,60,28,800,INK);drawSubtitleBlock(m.visual.subtitle,290,350,339,480,23,18,rgba(INK,.99));text(String(m.temps.minC)+'° — '+String(m.temps.maxC)+'°',980,305,46,600,INK,'right');}
function drawStoryHours(slots,icons){const x=44,y=520,w=992,h=704;box(x,y,w,h);const colW=w/5;const rowTop=[548,894],rowBottom=[872,1198];separator(x+20,886,x+w-20,886);for(let row=0;row<2;row++){for(let i=1;i<5;i++){const sx=x+colW*i;separator(sx,rowTop[row]+18,sx,rowBottom[row]-18);}for(let c=0;c<5;c++){const idx=row*5+c,item=slots[idx];const cx=x+colW*(c+.5),base=rowTop[row];text(String(item.hour).padStart(2,'0')+'h',cx,base+52,26,600,INK,'center');if(!item.missing&&icons[idx])drawImageCentered(icons[idx],cx,base+141,126,102);text(String(item.temperatureC)+(item.missing?'':'°'),cx,base+242,43,700,INK,'center');}}}
function drawFeedHours(slots,icons){const x=50,y=426,w=980,h=500;box(x,y,w,h);const colW=w/5;const rowTop=[446,690],rowBottom=[666,910];separator(x+20,678,x+w-20,678);for(let row=0;row<2;row++){for(let i=1;i<5;i++){const sx=x+colW*i;separator(sx,rowTop[row]+10,sx,rowBottom[row]-10);}for(let c=0;c<5;c++){const idx=row*5+c,item=slots[idx];const cx=x+colW*(c+.5),base=rowTop[row];text(String(item.hour).padStart(2,'0')+'h',cx,base+32,22,600,INK,'center');if(!item.missing&&icons[idx])drawImageCentered(icons[idx],cx,base+95,100,82);text(String(item.temperatureC)+(item.missing?'':'°'),cx,base+186,36,700,INK,'center');}}}
function drawStoryComments(){const x=44,y=1263,w=992,h=172;box(x,y,w,h);const main=normalizeText(m.visual.primaryLine),secondary=normalizeText(m.visual.secondaryLine);const mainSize=fittedFontSize(main,910,34,28,600);ctx.save();font(mainSize,600);const mainWidth=ctx.measureText(main).width;ctx.restore();if(mainWidth<=910)text(main,540,1345,mainSize,600,INK,'center');else wrap(main,540,1329,910,36,27,600,INK,'center',2);if(secondary){const secondarySize=fittedFontSize(secondary,900,26,22,600);ctx.save();font(secondarySize,600);const secondaryWidth=ctx.measureText(secondary).width;ctx.restore();if(secondaryWidth<=900)text(secondary,540,1401,secondarySize,600,rgba(INK,.99),'center');else wrap(secondary,540,1389,900,31,22,600,rgba(INK,.99),'center',2);}}
function drawFeedComments(){const x=50,y=955,w=980,h=120;box(x,y,w,h);const main=normalizeText(m.visual.primaryLine),secondary=normalizeText(m.visual.secondaryLine);const mainSize=fittedFontSize(main,900,29,24,600);ctx.save();font(mainSize,600);const mainWidth=ctx.measureText(main).width;ctx.restore();if(mainWidth<=900)text(main,540,1003,mainSize,600,INK,'center');else wrap(main,540,994,900,29,23,600,INK,'center',2);if(secondary){const secondarySize=fittedFontSize(secondary,890,21,18,550);ctx.save();font(secondarySize,550);const secondaryWidth=ctx.measureText(secondary).width;ctx.restore();if(secondaryWidth<=890)text(secondary,540,1042,secondarySize,550,rgba(INK,.99),'center');else wrap(secondary,540,1035,890,25,18,550,rgba(INK,.99),'center',2);}}
function drawStorySolar(solarIcons){const x=44,y=1479,w=992,h=279;box(x,y,w,h);const colW=w/5;for(let i=1;i<5;i++)separator(x+colW*i,y+28,x+colW*i,y+h-28);const defs=[['AUBE','dawn',m.solar.dawn,null],['LEVER','sunrise',m.solar.sunrise,m.solar.sunriseDeltaMinutes],['MIDI SOLAIRE','noon',m.solar.solarNoon,null],['COUCHER','sunset',m.solar.sunset,m.solar.sunsetDeltaMinutes],['CRÉPUSCULE','dusk',m.solar.dusk,null]];defs.forEach((def,i)=>{const cx=x+colW*(i+.5);text(def[0],cx,1547,18,700,INK,'center');drawImageCentered(solarIcons[def[1]],cx,1634,106,80);text(def[2]||'—',cx,1716,33,600,INK,'center');const shift=solarShiftLabel(def[3]);if(shift)text(shift,cx,1748,19,600,rgba(INK,.88),'center');});}
...
function drawStorySignature(){text('Ici, aujourd’hui.',540,1834,22,500,INK,'center');ctx.save();ctx.strokeStyle=GOLD;ctx.lineWidth=1.4;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(514,1858);ctx.lineTo(566,1858);ctx.stroke();ctx.restore();}
...
function renderStory(bg,mainIcon,slots,hourIcons,solarIcons){ctx=storyCtx;ctx.clearRect(0,0,1080,1920);drawCover(bg,1080,1920);drawHeader();drawStoryGeneral(mainIcon);drawStoryHours(slots,hourIcons);drawStoryComments();drawStorySolar(solarIcons);drawStorySignature();}
function renderFeed(bg,mainIcon,slots,hourIcons,solarIcons){ctx=feedCtx;ctx.clearRect(0,0,1080,1440);drawCover(bg,1080,1440);drawFeedHeader();drawFeedGeneral(mainIcon);drawFeedHours(slots,hourIcons);drawFeedComments();drawFeedSolar(solarIcons);drawFeedSignature();ctx=storyCtx;}
async function draw(){window.__LOKA_RENDER_STATUS={started:true,storyRendered:false,feedRendered:false,error:null};const slots=pickHourlySlots(m.hourly);const bg=await load(m.masterUrl);const mainIcon=await load(m.scene.pictogramUrl);const hourIcons=await Promise.all(slots.map(item=>load(item.pictogramUrl)));const solarIcons={dawn:await load(m.solarPictograms.dawn),sunrise:await load(m.solarPictograms.sunrise),noon:await load(m.solarPictograms.noon),sunset:await load(m.solarPictograms.sunset),dusk:await load(m.solarPictograms.dusk)};renderStory(bg,mainIcon,slots,hourIcons,solarIcons);window.__LOKA_RENDER_STATUS.storyRendered=true;renderFeed(bg,mainIcon,slots,hourIcons,solarIcons);window.__LOKA_RENDER_STATUS.feedRendered=true;window.__LOKA_RENDER_STATUS.rendered=true;window.__LOKA_EDITORIAL_AUDIT={sceneId:m.scene.id,title:m.scene.title,rendererReclassified:false};window.__LOKA_PICTOGRAM_AUDIT={version:m.pictogramLibrary.version,ink:m.pictogramLibrary.ink,gold:m.pictogramLibrary.gold,mainSceneId:m.scene.id};}
