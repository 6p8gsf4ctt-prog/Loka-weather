...
.badge{font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:#24653a;background:#e8f3ea;padding:6px 8px;border-radius:999px}
...
<div class="wrap">
  <div class="toolbar">
    <div class="topline">
      ...
      <div class="badge">Météo officielle · V24</div>
    </div>
  ...
  <div class="note">Le visuel suit uniquement la météo officielle effectivement publiée par LOKA.</div>
</div>

<script>
const p=${payload};
const cfg=${cfg};
const solar=cfg.solar||{dawn:null,sunrise:null,solarNoon:null,sunset:null,dusk:null,daylightMinutes:null,daylightDeltaMinutes:null};
...
async function draw(){
  const s=p.scene;
  const img=await load(s.masterUrl);
  const dark=[10,12,13,20,22,24].includes(s.id);
  ...
  text(String(p.city||'Tarnos').toUpperCase(),540,72,24,560,ink,'center');
  text(dateLabel(p.date),1030,72,20,470,ink,'right');

  text(s.label,540,205,66,760,ink,'center');
  wrap(
    p.editorial.mainVerdict||
    p.editorial.subtitle||
    '',
  ...
  const lines=Array.isArray(p.editorial.summaryLines)
    ? p.editorial.summaryLines.slice(0,3)
    : [];
  ...
  document.getElementById('summary').textContent=
    String(p.city||'Tarnos')+
    ' · '+
    s.label+
    ' · '+
    String(p.temperatures.minC)+
    '° / '+
    String(p.temperatures.maxC)+
    '°'+
    sunText;
}
function pngFile(){
