...
export function renderInstagramOfficial24(
  ...
  const model = {
    city: p.city,
    date: p.date,
    sceneLabel: p.scene.label,
    masterUrl: p.scene.masterUrl,
    subtitle: p.editorial?.mainVerdict || p.editorial?.subtitle || "",
    tempMinC: p.temperatures?.minC,
    tempMaxC: p.temperatures?.maxC,
    hourly: p.hourly,
    summaryLines: p.editorial?.summaryLines,
    mainVerdict: p.editorial?.mainVerdict,
    rainVerdict: p.editorial?.rainVerdict,
    notableEvent: p.editorial?.notableEvent
  };
